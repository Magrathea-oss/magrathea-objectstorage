...
public class InMemoryReactiveBucketRepository implements BucketCommandRepository, BucketQueryRepository {
    ...
    public Mono<CommandResult<Bucket>> delete(Bucket bucket) {
        return Mono.defer(() -> {
            ...
            if (removed == null) {
                return Mono.error(new com.example.magrathea.objectstore.reactive.repository.application.BucketNotFoundException(bucket.name()));
            }
    ...
    public Mono<Void> saveAbacConfiguration(String bucketName, AbacConfiguration config) {
        return findByName(bucketName)
            ...
            .switchIfEmpty(Mono.<Void>error(new com.example.magrathea.objectstore.reactive.repository.application.BucketNotFoundException(bucketName)));
    }
