...
### PA-3 — Reactive Pipeline Refactor ✅ Implemented and validated

| Field | Plan / Result |
|---|---|
| Focus | Replace the monolithic orchestrator flow with a staged reactive read/write pipeline. |
| Current status | `@implemented-and-validated`. The staged pipeline, bounded upload/read demand, single-pass filesystem GetObject, deterministic write-failure cleanup, client cancellation, event publication, conditional chunking, typed whole-object persistence, physical EC shards, schema-3 reconstruction facts, and local bounded EC 4+2 decoding are validated by pipeline-unit, WebTestClient, runtime, and static-architecture Cucumber modes. |
| Validation evidence | Feature: `phase-3-reactive-pipeline.feature`. Existing pipeline modes remain green; focused `ReqPipeline017EcReconstructionCucumberTest` passes **5 scenarios / 46 steps**, including all **15 four-of-six** EC 4+2 survivor combinations. `Phase3PipelineUnitSpecsCucumberTest` remains green for the `REQ-PIPELINE-014/015` whole-object and physical-shard regressions. REQ-PIPELINE-007..012 remain Cucumber-validated by the static architecture runner and REQ-PIPELINE-013 by its runtime boundary runner. |
| Remaining gap | None in the declared local PA-3/EP-3 scope. `REQ-PIPELINE-017` is output-only: repair publication, a scanner/self-healing daemon, distributed shard placement/transfer, Ratis repair ownership, rebalance, cleanup, and ADR 0030 faults remain later EP-10 slices. External observability completion remains PA-4. |
...
| Remaining gaps | PA-6 itself remains modeled/domain-only. EP-10 separately implements fixed A/B/C consensus publication, direct whole-object replication, bounded current-generation repair, and bounded periodic current-reference anti-entropy. Local output-only EC reconstruction is separately validated by `REQ-PIPELINE-017`, and fixed A/B/C EC 4+2 placement/transfer is validated by `REQ-CLUSTER-015`; clustered S3 EC reads, EC self-healing, parameterized EC, dynamic membership, rebalance, automated orphan cleanup, wider healing/topologies, and general multi-node readiness remain absent. Distributed production readiness is not claimed; PA-6 planner output is never execution evidence. |
...
Done: all four acceptance gates met — `YamlStoragePolicyCatalog`/`YamlStorageDeviceCatalog`/`YamlDiskSetCatalog` with robust validation (36 catalog tests); `MINIO_STANDARD` classpath YAML with deterministic plans (`MinioStandardIntegrationTest`); fail-fast on missing external config; context tests for mutually exclusive backends; Admin API integrated via the same catalog beans (`AdminRouterTest`, 9 tests). Local physical EC shards and output-only reconstruction are now separately validated by `REQ-PIPELINE-015/017`. Remaining: no integration test exercises the Admin API through a real YAML-backed Spring Boot context; fixed A/B/C EC 4+2 placement/transfer is runtime-validated internally but clustered S3 EC reads, self-healing, parameterized EC, and wider topology remain absent; policy schema versioning is documented but not enforced.
...
| Status | `@implemented-and-validated` (write path static streaming constraints `REQ-PIPELINE-007/008`, non-range GetObject streaming `REQ-PIPELINE-009`, ranged GetObject streaming `REQ-PIPELINE-010`, multipart UploadPart/UploadPartCopy and part-store static streaming constraints `REQ-PIPELINE-011/012`, completed multipart object assembly `REQ-S3-002-D`, UploadPartCopy copied-byte assembly `REQ-S3-002-F`, multipart XML error semantics `REQ-S3-002-G..K`, and multipart part-body full-process restart completion `REQ-S3-002-E` are `@implemented-and-validated`. The staged-pipeline Cucumber runner now validates REQ-PIPELINE-001 canonical UUID/checksum publication and post-object-commit content-address publication in pipeline-unit and WebTestClient modes, REQ-PIPELINE-004 filesystem cleanup for chunk, manifest, and upstream-body failures, REQ-PIPELINE-005 pipeline-unit plus client-driven loopback HTTP cancellation cleanup owned by Reactor `usingWhen`, and REQ-PIPELINE-006 instrumentation. REQ-PIPELINE-002 is implemented-and-validated in pipeline-unit and WebTestClient modes: both execute a deterministic 256 MiB upload through the production staged pipeline and real filesystem with measured live source-buffer retention, bounded request signals, manifest/order/length checks, exact streamed readback, and anti-aggregation guards. The WebTestClient mode selects the dedicated 1 MiB-chunk `PIPELINE` test policy through `x-amz-storage-class`; the adapter requests three upstream buffers per batch so transport plus active processing remains within the declared four-buffer ceiling. REQ-PIPELINE-003/004/005 are implemented-and-validated in their declared pipeline-unit and WebTestClient modes. REQ-PIPELINE-003 executes the deterministic 256 MiB fixture without test-side retention: payload files are opened once and emitted as bounded 64 KiB blocks after metadata validation, with no complete-payload preflight or second filesystem read. Upload commit re-reads temporary files incrementally and rejects mismatches before atomic publication (REQ-PIPELINE-016); clients validate post-commit GetObject integrity through the committed checksum/ETag contract. `REQ-PIPELINE-013` supplies finite-demand adapter-boundary evidence only. ADR 0025 and new REQ-PIPELINE-014/015 correct the storage taxonomy: plain writes must remain whole-object units, while only multipart, dedup, and EC may produce chunks/shards. REQ-PIPELINE-014 is implemented-and-validated in pipeline-unit and WebTestClient modes: a plain 8 MiB upload remains one `FileUnit`, records `whole-object-pass-through`, writes a schema-2 typed `WHOLE_OBJECT` manifest, persists only under `whole-objects`, and reads back exactly; schema-0/1 chunk-only manifests remain readable as `LEGACY_CHUNK`. REQ-PIPELINE-015 is implemented-and-validated in both modes: an 8 MiB EC_4_2 upload creates two bounded 4 MiB stripes, eight ordered data shards and four parity shards with typed manifests, checksum validation, and exact readback. ADR 0032 and all five `REQ-PIPELINE-017` scenarios are implemented-and-validated by the focused **5-scenario / 46-step** gate: new EC writes use schema 3 with explicit stripe/shard/k/m/parity/logical-length/stored-length/SHA-256/location facts; all **15 four-of-six** survivor combinations reconstruct both omitted shards; short-stripe output and workspace remain bounded; invalid input fails closed; and GF(256) work runs on the injected one-worker/16-queue scheduler. The decoder returns verified local output only and publishes nothing. All declared EP-3 scenarios are implemented-and-validated; distributed placement/transfer, self-healing/repair publication, rebalance, cleanup, and chaos remain later scopes.) |
...
| Status | `@implemented-and-validated` for the declared single-node EP-4 scope. REQ-SCRUB-001/002 provide opt-in periodic transform-aware integrity scrubbing with report-only/quarantine policies. REQ-GC-001..004 provide durable typed reclamation for whole objects, shared dedup chunks, multipart abort/expiry, and EC shards; delete and overwrite prepare restart-safe reclamation markers before reference detachment, and retries are idempotent. REQ-QUOTA-001/002 provide durable logical-byte quota accounting, atomic process-local concurrent reservations, restart reconciliation, and Admin quota/capacity reporting. REQ-CAPACITY-001 maps deterministic and recognized native ENOSPC failures to S3 `InsufficientStorage`, cleans unpublished state, and emits redacted capacity observability fields. All declared S3 capacity scenarios pass WebTestClient and real AWS CLI modes. Multi-node quota coordination belongs to EP-10; EC repair/reconstruction and deep plaintext decrypt/decompress validation are not claimed by this EP-4 scope. |

### EP-5 — Operability & Delivery (HIGH)

| Field | Plan |
|---|---|
| Focus | Declared single-node preview operability and JVM delivery baseline: appendix/source-hygiene checks, focused Cucumber/security/semantic/EP-5 gates, root JVM Docker build+smoke, manual native Docker build+smoke, packaged single-node containers using storage-engine by default, Admin API liveness/readiness probes, SIGTERM shutdown validation for committed state plus single, multipart, and two-request concurrent streaming writes, first offline backup/restore rehearsal, first single-node DR rehearsal with declared RTO/RPO, and first SLO/alerting rule bundle. Optional deployment integrations and distributed/scale concerns are tracked in their owning later phases. |
| Owner agents | `java-scaffolder`, `java-infra-coder`, `documenter`, `java-tester` |
| Requirement feature files | `requirements/phase-ep5-operability.feature`, `specs/phase-ep5-storage-migration.feature`, and `specs/phase-ep5-delivery.feature` |
...
| Status | `@implemented-and-validated` for the declared `0.1.x` single-node envelope. REQ-PERF-001..008 validate 256 MiB single-object and assembled-multipart limits, 64 MiB part limits, incremental over-limit rejection, request timeout cleanup, fail-fast concurrency, token-bucket rate limiting, and a real 64-connection process cap. REQ-PERF-009..013 validate an eight-worker 45-second mandatory CI load and an actually executed 15-minute soak under `-Xmx256m`, bounded heap/idle resources, redacted metrics, reproducible JSON evidence, and an explicit non-benchmark disclaimer. Local evidence: CI 184 operations, p99 204 ms, peak heap 179,890,176 bytes; soak 3,600 operations, p99 187 ms, peak heap 138,494,976 bytes; zero corruption/unexpected responses. KA-6 exclusively owns competitive benchmarks and portable performance claims. |
...
| Field | Plan |
|---|---|
| Focus | Accepted cluster architecture contract (how, not whether) plus reproducible supply-chain evidence wiring. Networked cluster execution remains EP-10. |
| Owner agents | `arc42`/ADR documentation, supply-chain implementation, and Cucumber validation |
| Requirement feature file | `specs/phase-ep8-cluster-architecture-supply-chain.feature` |
| Key requirement IDs | `REQ-HA-001..008`, `REQ-SUPPLY-001..006` |
| Accepted architecture | ADR 0027 is accepted for architecture only: internal protobuf gRPC/mTLS, consensus-authoritative membership/metadata, direct quorum data transfer, PA-6 policy reuse, `N=3/W=2` without degraded writes, authoritative-generation reads, and additive parent-linked topology. This acceptance is not cluster runtime evidence. |
| Clean evidence identity | Full revision `209b3170b64d3311ba9b773fdb7bd5581519e682`; local image ID `sha256:e5b5862948853a613f8b92adc8135fa81b84bdd9f36c9ae8d69b9a6bd6dd7966`; evidence is explicitly unpublished. |
| Supply-chain evidence | CycloneDX 1.6 application JSON: 102 components/103 dependencies, SHA-256 `4025d7206af01683da5cffa84862f20f68c7568dd9338ee9920d55947df545d1`; XML SHA-256 `b934de56a243cf6b5464566f45635e1f6ac0fc244d2ceb7c1f7418308012952f`. License inventory: 85 recognized, 13 unknown, 4 ambiguous, SHA-256 `0df3a3eac37e554dce6d67f227ffead9b28675f47fca9dab741d6850ac47b214`; it makes no compliance claim. Syft 1.31.0 image `sha256:c15fa8af4c25edd72c0daf026d095fe51adbcfc7ad5d79a66e93d88f249e5abb` produced an image CycloneDX inventory with 7,105 components, SHA-256 `8d13ae409ef7230d52b4d850d0700db69132297910e55cffdbe27c08e28a90ba`. |
| Runtime hardening evidence | Passed as UID/GID 10001 with read-only root, explicit data/tmp mounts, `no-new-privileges`, capability drop `ALL`, no host PID/IPC/network/UTS namespaces, and no container-engine socket. User-namespace remapping was unavailable and is recorded as such. Admin readiness reported `storage-engine`; an exact 129-byte object, checksum, and ETag persisted across container replacement. |
...
| Validation evidence | Architecture Cucumber: 13/13 expanded scenarios, 103 steps. Supply-chain Cucumber: 9/9 expanded scenarios, 103 steps. Appendix freshness and deterministic diff checks pass. |
| Status | `@implemented-and-validated` for the bounded architecture/evidence wiring only. EP-10 has since implemented a partial fixed-cluster runtime, bounded repair, and bounded periodic current-reference discovery; broader membership, wider topology/healing execution, rebalance, cleanup, and distributed production readiness remain outside EP-8 and retain their exact EP-10 statuses. |
...
| Validation modes | Shared Java 21 real-process multi-node WebTestClient/AWS CLI runner for externally observable S3 behavior; focused real Ratis control/snapshot/mTLS and gRPC data-plane Ability runners; repository-rooted Maven dependency, JDK AST, proto3, layering-script, and isolated fail-closed probe validation for the internal `REQ-CLUSTER-014` architecture contract; focused Ratis current-reference paging, real-filesystem, grpc-java/mTLS, lifecycle, failure, race, and counter validation for `REQ-CLUSTER-027`; and explicit interruption/fault matrices for each claimed runtime boundary. Local prerequisite `REQ-PIPELINE-017` uses its separate focused pipeline-unit runner and is not distributed EP-10 evidence. Docker/Testcontainers may supplement but do not replace the observed real-process gates. |
...
| Acceptance gates | Cluster behavior is proven only by the agreed multi-node and focused mechanism modes; no distributed claim broadens beyond explicit requirement status. The bounded `REQ-CLUSTER-024` gate requires all seven real-filesystem/gRPC interruption points, not only control-state simulation. `REQ-CLUSTER-027` requires both bounded current-reference discovery scenarios and all 36 steps, including exclusive paging, fixed-node named-obligation probes, lifecycle/failure/race behavior, existing fenced repair, and no-overlap counters. `REQ-CLUSTER-014` has a separate internal architecture-only validation mode and cannot provide S3 or runtime-side-effect evidence. None of these gates proves broader chaos or cluster readiness. |
| Runtime validation evidence | The historical 2026-07-13 repair-control result remains **22 scenarios / 210 steps**. On **2026-07-14**, focused `ReqCluster024CucumberTest` passed **7 / 168** and focused `ReqCluster027CucumberTest` passed exactly **2 / 36**. The existing validation set also passed expanded repair-control **22 / 294**, data-plane **4 / 40**, and control/TLS regressions plus module compilation, layering, source hygiene, and diff checks. The `024` harness independently crashed/restarted B with distinct PIDs and its original non-empty roots; the `027` gate used real three-voter Ratis paging, real filesystem targets, and actual grpc-java UUID-bound mTLS repair. |
| Architecture-boundary evidence (2026-07-14) | `ReqCluster014ArchitectureContractCucumberTest` passed **1 scenario / 17 steps**. It parsed **17 reactor modules**, **87 direct non-test Maven dependencies** plus **2 profile dependency declarations**, **473 tracked production Java sources** with JDK AST parsing, and **1 versioned proto3 contract**; executed `scripts/check-module-layering.sh`; and passed **12 missing/empty/malformed** plus **4 supported POSIX unreadable** fail-closed probes with exact-path diagnostics and protected-input hash preservation. Coordination **7/54**, control **2/19**, data **4/40**, repair **22/294**, EP-8 architecture **13/103**, source hygiene, and appendix freshness regressions passed. CI is wired but has not run. This is source/build-boundary evidence only. |
| Status | `@partial` — fixed A/B/C baseline, bounded current-generation repair/discovery, and fixed distributed EC 4+2 placement/transfer are validated. `REQ-CLUSTER-001..005`, `008..015`, and `019..027` are implemented-and-validated, with `014` architecture-only, `015` limited to internal placement/transfer/publication, and `027` limited to its two fixed A/B/C discovery scenarios; broad cluster `017` remains partial; `006/007/016/018` remain not implemented. Clustered S3 EC reads, EC self-healing, parameterized EC, rebalance, and cleanup remain absent. EP-10 remains the active implementation phase. |

EP-10 continuation remains explicit and requirement-first. ADR 0032 closes only local output-only decoding; the next separate slices are:

| Order | Next bounded slice | Current status / gate |
|---|---|---|
| **1 — complete** | Fixed one-stripe distributed EC 4+2 shard placement and direct transfer (`REQ-CLUSTER-015`) | `@implemented-and-validated` for the internal fixed A/B/C mechanism: **5 scenarios / 40 steps**, deterministic two-shards-per-node placement, real grpc-java/mTLS transfer, six-ack Ratis publication, fail-closed evidence, and restart recovery. No clustered S3 EC read or healing claim. |
| **2 — next** | Early bounded ADR 0030 kernel and committed-shard fault subset | Planned test-tooling slice after distributed placement/transfer, constrained to EC 4+2 only: no-op-default deterministic plan/evidence kernel plus exact committed-shard unavailable/corruption actions, dual non-production activation, finite blast radius, quarantine/restoration, and redacted evidence. No implementation or fault evidence is claimed. |
| **3** | EC monitoring and self-healing | Requirement-first scanner/daemon slice: process-local detection may schedule work, but Ratis must own canonical stripe/shard repair jobs, claims, retries, fencing, and results; verified atomic shard publication is required. |
| **4** | Shard rebalance | Separate placement-changing slice with authority, currentness, fencing, capacity, and failure evidence. Broad `REQ-CLUSTER-017` remains partial. |
| **5** | Fenced orphan cleanup | Separate destructive slice defining prepared/replaced shard reachability, retention, deletion fences, restart behavior, and audit evidence. |
| **Final** | Generalized ADR 0030 chaos | Broader pipeline/cluster actions and realistic chaos remain final after normal paths and semantic/resource gates are stable; the early EC test subset is not generalized chaos. |

Whole-object `N=3/W=2` replication remains valid for deployments below the arranged distributed-EC minimum (currently three nodes/failure domains for the intended EC 4+2 layout) and is not removed by the EC priority. Broader whole-object convergence remains later work and may become a substrate for inter-cluster replication. Inter-cluster and NAS-backed replication are deferred and NAS-dependent because meaningful validation requires suitable NAS infrastructure.
...
| **9 — active** | **EP-10 S3 cluster** | **High** | **`@partial`; fixed EC 4+2 placement/transfer (`REQ-CLUSTER-015`) is complete; next implement the bounded ADR 0030 committed-shard test subset, EC self-healing/read integration, shard rebalance, and fenced cleanup; parameterized EC and generalized chaos remain later/final** |
...
### Definition of Production Ready (EP-0 checklist)
...
- [x] EP-3: GetObject and multipart paths stream with bounded memory; filesystem payload artifacts are opened once and emitted in 64 KiB blocks after metadata-only validation; uploads verify temporary-file bytes before atomic publication; pipeline-unit/WebTestClient cover stage ordering, 256 MiB demand, failure/cancellation cleanup, whole-object namespaces, physical EC shards, and exact readback; focused `REQ-PIPELINE-017` covers schema-3 facts plus local bounded output-only reconstruction (5 scenarios / 46 steps and all 15 four-of-six combinations); runtime/static guards prevent aggregation regressions.
- [x] EP-4: typed whole-object/segmented-artifact GC, dedup reachability accounting, multipart/EC reclamation, logical-byte quotas, concurrent reservations, ENOSPC behavior, and transform-aware scrubbing are validated for the declared single-node scope.
...
| MinIO semantic ambiguity | `MINIO_STANDARD` interpreted inconsistently | CC-5 fixed the documented semantics; local physical EC shards and output-only decoding are now tested, while distributed shard placement/transfer remains explicitly unimplemented | `java-domain-coder`, `documenter` |
