...
- [ ] EP-3: GetObject and multipart paths stream with bounded memory; architecture tests guard all fixed paths. Pipeline-unit/WebTestClient Cucumber now covers selected stage ordering, canonical chunk publication, failure/cancellation cleanup, and instrumentation; REQ-PIPELINE-002 adds deterministic 256 MiB pipeline-unit and WebTestClient bounded-demand/live-buffer evidence, REQ-PIPELINE-005 adds client-driven loopback HTTP cancellation, while `REQ-PIPELINE-013` supplies finite-demand adapter-boundary evidence. The GetObject integrity-preflight latency/single-pass trade-off remains outstanding, so this gate is not closed.
