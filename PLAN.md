...
- **Handler-local state in the web adapter (planner-verified 2026-07-02; partially remediated 2026-07-03):** `S3BucketConfigHandler` originally held **6 `ConcurrentHashMap`s directly in the WEB ADAPTER**. Object-lock, inventory-table and journal-table have been moved onto the `Bucket` aggregate (durable via `BucketStore`), leaving **3 remaining** handler-local maps: abacConfigs, metadataConfigs, metadataTableConfigs (`@config-only`). Remaining layering/durability gap tracked in EP-2 and the Ancillary S3 API Requirements Backlog.
...
| Status | `@partial` — **major progress 2026-07-03**: bucket registry (full `BucketConfig` document), multipart upload state (upload id + recorded parts), and per-object configuration (legal hold, lock config, retention, encryption, restore) are now durable JSON documents under `metadata/buckets`, `metadata/multipart-uploads`, `metadata/object-config` with crash-safe atomic commits (`BucketStore`, `MultipartUploadStateStore`, `ObjectConfigMetadataStore` + shared `DurableJson`). **Handler-local families reduced 6→3**: bucket object-lock, inventory-table and journal-table configuration moved from `S3BucketConfigHandler` in-memory maps onto the `Bucket` aggregate (`BucketConfig` +3 fields/VOs, persisted through `updateBucket` → durable via `BucketStore`); validated by `MetadataDurabilityRestartTest$BucketConfigFamilies`. Validated overall by `MetadataDurabilityRestartTest` (7 restart-simulation unit tests) and the upgraded `StorageEngineRestartSafetyTest` (real context restart: HeadBucket 200, ListBuckets includes bucket, re-create → 409). REQ-DUR-001 `@implemented-and-validated`; REQ-DUR-002/003 + notification/object-lock/inventory-table/journal-table `@implemented-not-e2e-validated`. **Open:** object tags + ACL durability (`@partial`), the remaining 3 handler-local families — ABAC, metadata config, metadata-table (`@config-only`), the combined REQ-DUR-005 scenario, and Cucumber runner wiring for the EP-2 feature. Test-glue restart semantics split: `reset()` (wipe) vs `reloadFromDisk()` (restart simulation). |
