...
- [ ] EP-5: CI pipeline green on full gate + appendix check + docker build; offline backup/restore, first SIGTERM shutdown validation, and first single-node DR RTO/RPO rehearsal exist; online/multi-node DR and broader in-flight draining still pending.
