...
| Status | `@partial`: CI packaging gates are wired; packaged JVM/native single-node containers activate storage-engine and package YAML catalogs so `/admin/ready` is expected to be ready; Admin API `/admin/live` + `/admin/ready` are implemented and Cucumber-validated as `REQ-OPS-001..003`, including fail-closed readiness when required catalogs are unavailable; `REQ-OPS-004` validates that a storage-engine process exits after SIGTERM without forced termination and preserves a committed S3 object across restart; `REQ-OPS-009` validates that SIGTERM drains an active 524,288-byte streaming PutObject before exit and preserves its byte count/checksum across restart, with `docs/runbooks/graceful-shutdown.md` documenting the procedure; `REQ-OPS-005` validates offline storage-root backup/restore after simulated primary data loss and `docs/runbooks/backup-restore.md` documents the procedure scope; `REQ-OPS-006` validates declared single-node offline DR objectives (RTO 30 seconds, RPO last completed offline backup) and `docs/runbooks/disaster-recovery.md` documents the rehearsal; `REQ-OPS-007` validates object-manifest schema versioning/migration compatibility and `docs/runbooks/schema-migration.md` documents the current contract; `REQ-OPS-008` validates a shipped Prometheus/Loki SLO and alerting starter bundle with `docs/runbooks/slo-alerts.md`; broader metadata-family schema contracts, alert delivery through live monitoring systems, broader runbooks, online/incremental DR, multi-node DR, concurrent/multipart in-flight draining evidence, and load-level shutdown validation remain. |
...
- [ ] EP-5: CI pipeline green on full gate + appendix check + docker build; offline backup/restore, SIGTERM shutdown validation covers committed state and one active streaming PutObject, first single-node DR RTO/RPO rehearsal, object-manifest schema-versioning, and first SLO/alerting rule bundle exist; online/multi-node DR, live alert delivery, broader metadata schema contracts, and concurrent/multipart draining still pending.
