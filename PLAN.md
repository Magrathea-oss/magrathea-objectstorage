...
- [ ] EP-5: CI pipeline green on full gate + appendix check + docker build; offline backup/restore, first SIGTERM shutdown validation, first single-node DR RTO/RPO rehearsal, and object-manifest schema-versioning exist; online/multi-node DR, broader metadata schema contracts, and broader in-flight draining still pending.
