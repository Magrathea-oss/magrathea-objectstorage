...
- [ ] EP-5: CI pipeline green on full gate + appendix check + docker build; offline backup/restore, first SIGTERM shutdown validation, first single-node DR RTO/RPO rehearsal, object-manifest schema-versioning, and first SLO/alerting rule bundle exist; online/multi-node DR, live alert delivery, broader metadata schema contracts, and broader in-flight draining still pending.
