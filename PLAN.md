...
| Status | `@partial` — fixed A/B/C baseline and bounded current-generation repair are validated. `REQ-CLUSTER-001..005`, `008..014`, and `019..026` are implemented-and-validated, with `014` limited to its internal architecture-only mode; broad `017` remains partial; `006/007/015/016/018` remain not implemented. EP-10 remains the active implementation phase. |

EP-10 continuation is explicit and requirement-first. None of the listed `REQ-CLUSTER-017` sub-capabilities is implemented:

| Order | Next bounded slice | Current status / gate |
|---|---|---|
| **1** | Split `REQ-CLUSTER-017`: broad/periodic discovery and anti-entropy | Planned requirement-first slice; define bounded discovery, scheduling, and observable outcomes before implementation. No implementation claim; broad `017` remains partial. |
| **2** | Split `REQ-CLUSTER-017`: rebalance | Planned requirement-first slice; define placement-change authority, fencing, and evidence independently. No implementation claim; broad `017` remains partial. |
| **3** | Split `REQ-CLUSTER-017`: fenced orphan cleanup | Planned requirement-first slice; define prepared-artifact reachability, retention, and deletion fences independently. No implementation claim; broad `017` remains partial. |
| **4** | Later explicit requirements | `006/007/015/016/018` remain not implemented and require separate requirement-first slices. |
...
| **9 — active** | **EP-10 S3 cluster** | **High** | **`@partial`; resume with separate requirement-first `REQ-CLUSTER-017` slices in this order: broad/periodic anti-entropy, rebalance, then fenced orphan cleanup; no `017` sub-capability is implemented** |
