...
| Current status | `@implemented-and-validated`. The staged pipeline, bounded upload/read demand, single-pass filesystem GetObject, deterministic write-failure cleanup, client cancellation, event publication, conditional chunking, typed whole-object persistence, and physical EC shards are validated by pipeline-unit, WebTestClient, runtime, and static-architecture Cucumber modes. |
...
| Remaining gap | None in the declared PA-3/EP-3 scope. Periodic at-rest scrubbing/repair and degraded EC reconstruction belong to EP-4, while external observability completion remains PA-4. |
...
| Remaining gaps | PA-6 itself remains modeled/domain-only. EP-10 separately implements fixed A/B/C consensus publication, direct replication, bounded current-generation repair, and bounded periodic current-reference anti-entropy, but not dynamic membership, EC transfer, rebalance, automated orphan cleanup, wider healing/topologies, or general multi-node readiness. Distributed production readiness is not claimed; PA-6 planner output is never execution evidence. |
...
| Status | `@partial` — fixed A/B/C baseline, bounded current-generation repair, and bounded periodic current-reference anti-entropy are validated. `REQ-CLUSTER-001..005`, `008..014`, and `019..027` are implemented-and-validated, with `014` limited to its internal architecture-only mode and `027` limited to its two fixed A/B/C discovery scenarios; broad `017` remains partial; `006/007/015/016/018` remain not implemented. EP-10 remains the active implementation phase. |

EP-10 continuation remains explicit and requirement-first. ADR 0031 and `REQ-CLUSTER-027` close only bounded periodic current-reference discovery; the next separate slices are:

| Order | Next bounded slice | Current status / gate |
|---|---|---|
| **1** | Split `REQ-CLUSTER-017`: rebalance | Next requirement-first slice; define placement-change authority, fencing, and evidence independently. No implementation claim; broad `017` remains partial. |
| **2** | Split `REQ-CLUSTER-017`: fenced orphan cleanup | Following requirement-first slice; define prepared-artifact reachability, retention, and deletion fences independently. No implementation claim; broad `017` remains partial. |
| **3** | Later explicit requirements | `006/007/015/016/018` remain not implemented and require separate requirement-first slices. |
| **Final, after normal EP work** | ADR 0030 deterministic chaos | Remains accepted as the deferred final chaos work after normal Storage Engine and EP-10 path closure; `027` makes no ADR 0030 execution claim. |
...
| **9 — active** | **EP-10 S3 cluster** | **High** | **`@partial`; bounded periodic current-reference anti-entropy is validated under `027`; resume with separate requirement-first `017` slices in this order: rebalance, then fenced orphan cleanup; wider healing/topologies remain absent** |
