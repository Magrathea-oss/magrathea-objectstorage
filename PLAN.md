...
> **2026-07-02 completion pass update.** This pass first closed documentation/build-plan items, then used Podman with `magrathea-build:fedora42` (JDK 21, Maven, Python, Node/npm, AWS CLI) after DNS/network recovery to execute Phase 1 runtime gates. Phase 1 REQ-UPLOAD-001/002 are closed through the plan-authorized validation-mode decision accepting bootstrap integration tests as the sole required runtime validation mode, with rationale in the feature file. REQ-UPLOAD-003 is not claimed as runtime-validated: its executable Cucumber scenario and bootstrap JUnit test were removed because they imposed obsolete non-dedup multi-chunk persistence, and Phase 1 runners no longer select it. **REQ-UPLOAD-004 has since been independently runtime-validated in both required modes** (2026-07-02, Podman image `magrathea-build:fedora42`, JDK 21, Maven 3.9.9, AWS CLI 2.34.29): `mvn -B --no-transfer-progress -pl s3-reactive-api-adapter -am test -Dtest=Phase1UploadStorageEngineCucumberTest,Phase1UploadStorageEngineAwsCliCucumberTest -Dsurefire.failIfNoSpecifiedTests=false` was run twice with identical results (BUILD SUCCESS, 28 scenarios discovered / 0 failures / 0 errors / 19 skipped per class), and the REQ-UPLOAD-004 scenario ran and passed independently in both `@webclient` and `@awscli` modes. The java-tester agent refreshed the feature-file tag from `@implemented-not-e2e-validated` to `@implemented-and-validated` and removed the feature-level `@partial` tag, since all five REQ-UPLOAD-001/002/004/005/006 scenarios in `phase-1-upload-storage-engine.feature` now carry `@implemented-and-validated`; the Gherkin appendix was regenerated and its freshness re-verified (`--check` OK, 66 scenarios from 6 feature files, `@implemented-and-validated` count 40→41, `@implemented-not-e2e-validated` count 18→17). The Gherkin appendix freshness check and deterministic regeneration are wired into the Docker documentation build, the local `--check` gate passes, the Admin UI screen plan (`docs/admin-ui-plan.md`) is delivered, and stale cross-phase checklist items were verified against code/docs and checked with evidence notes. Remaining non-Phase-1 gates must not be claimed until separately implemented/executed: Phase 3 external pipeline-event observability exposure plus `@not-implemented` removal and runner execution; Phase 4 Cucumber e2e runner creation/execution; Phase 5 REQ-S3-002-C full-restart e2e validation; Phase 6 real networked replication/membership/healing/rebalance execution and multi-node e2e validation; full `docker build` verification of the new appendix gate. No runtime test evidence was fabricated in this pass.
...
| Current status | `@implemented-and-validated` for REQ-UPLOAD-001 and REQ-UPLOAD-002 under the 2026-07-02 validation-mode decision: bootstrap JUnit integration validation is formally accepted as the sole required runtime validation mode for those two restart/reload requirements, with rationale documented in `phase-1-upload-storage-engine.feature`; their `@webclient`/`@awscli` examples remain supplementary and are excluded from the Phase 1 Cucumber runners by `not @bootstrap-integration-required`. REQ-UPLOAD-003 is no longer an executable Phase 1 target: its Cucumber scenario and bootstrap JUnit test were removed because they required obsolete non-dedup multi-chunk persistence. Phase 1 runners no longer select REQ-UPLOAD-003. REQ-UPLOAD-004 is now `@implemented-and-validated`: it was independently runtime-validated in both `@webclient` (`Phase1UploadStorageEngineCucumberTest`) and `@awscli` (`Phase1UploadStorageEngineAwsCliCucumberTest`) modes, 2026-07-02, Podman `magrathea-build:fedora42` (JDK 21, Maven 3.9.9, AWS CLI 2.34.29); the feature-level `@partial` tag was removed because no scenario in `phase-1-upload-storage-engine.feature` remains `@partial`, `@placeholder`, `@config-only`, or `@not-implemented`. REQ-UPLOAD-005 and REQ-UPLOAD-006 retain previous WebTestClient and AWS CLI Cucumber validation. |
| Validation evidence | Bootstrap integration evidence: `StorageEngineRestartSafetyTest` (REQ-UPLOAD-001), `StorageEngineHttpReadAfterWriteTest` manifest reload coverage (REQ-UPLOAD-002), `StorageEngineUploadAtomicityTest` bootstrap coverage for failed-upload atomicity (REQ-UPLOAD-004), `StorageEngineHttpReadAfterWriteTest` read-after-write coverage (REQ-UPLOAD-005), and `StorageEngineIntegrityDetectionTest` (REQ-UPLOAD-006). The removed REQ-UPLOAD-003 bootstrap test is not evidence. REQ-UPLOAD-004 is now additionally accepted as independently runtime-validated: `mvn -B --no-transfer-progress -pl s3-reactive-api-adapter -am test -Dtest=Phase1UploadStorageEngineCucumberTest,Phase1UploadStorageEngineAwsCliCucumberTest -Dsurefire.failIfNoSpecifiedTests=false` ran twice (before and after the feature-file tag edit) with identical BUILD SUCCESS results (28 scenarios discovered / 0 failures / 0 errors / 19 skipped per class), and its scenario executed and passed independently in both `@webclient` and `@awscli` modes, 2026-07-02, Podman `magrathea-build:fedora42` (JDK 21, Maven 3.9.9, AWS CLI 2.34.29). Local freshness gate passed: `python3 scripts/generate-gherkin-requirements-appendix.py --check` (66 scenarios from 6 feature files, re-verified after the tag refresh). |
| Acceptance gates | Not met for full Phase 1 because REQ-UPLOAD-003 is out of executable scope. Bootstrap integration gates cover REQ-UPLOAD-001/002 as the agreed required validation mode. REQ-UPLOAD-003 has been removed from executable Phase 1 scope because its multi-durable-chunk expectation contradicts the current architecture decision. REQ-UPLOAD-004 has met its e2e/runtime validation gate in both required modes. |
...
- [x] Add Phase 1 Cucumber WebTestClient runner glue so `phase-1-upload-storage-engine.feature` scenarios are selected from the shared feature. The current runner no longer selects REQ-UPLOAD-003; REQ-UPLOAD-001/002 supplementary examples are excluded by `@bootstrap-integration-required`; REQ-UPLOAD-004 has since been independently runtime-validated (see below).
- [x] Add Phase 1 AWS CLI Cucumber runner selection from the shared feature. The current runner no longer selects REQ-UPLOAD-003; REQ-UPLOAD-001/002 supplementary examples are excluded by `@bootstrap-integration-required`; REQ-UPLOAD-004 has since been independently runtime-validated in AWS CLI mode (see below).
- [x] Provide `fixtures/upload/large-object.bin` (deterministic 256 MiB binary) for REQ-UPLOAD-004 and the removed obsolete REQ-UPLOAD-003 example.
- [x] Runtime-validate REQ-UPLOAD-001 and REQ-UPLOAD-002 through the Phase 1 Cucumber WebTestClient and AWS CLI runners, or formally accept bootstrap integration tests as the sole validation mode and document the rationale in the feature file. **Done 2026-07-02 via the second branch:** bootstrap integration tests (`StorageEngineRestartSafetyTest`, `StorageEngineHttpReadAfterWriteTest` manifest-reload coverage) are formally accepted as the sole required runtime validation mode because both requirements assert full process stop/start against the same storage-engine filesystem root, which the in-process WebTestClient glue cannot perform without duplicating the bootstrap harness and which AWS CLI mode cannot drive without an externally restartable server process. Rationale is documented in the feature file; status tags updated to `@implemented-and-validated` + `@bootstrap-integration-required`.
...
- [x] Remove obsolete REQ-UPLOAD-003 executable validation. **Done 2026-07-02:** the Cucumber scenario and bootstrap JUnit test were removed because they imposed non-dedup multi-chunk persistence that the current architecture intentionally does not provide; Phase 1 runners no longer select REQ-UPLOAD-003.
- [x] Runtime-validate REQ-UPLOAD-004 independently through the Phase 1 Cucumber WebTestClient and AWS CLI runners in an environment with Java/Maven and `aws` for AWS CLI mode. Do not use stale REQ-UPLOAD-003 multi-chunk expectations as evidence. **Done 2026-07-02:** `mvn -B --no-transfer-progress -pl s3-reactive-api-adapter -am test -Dtest=Phase1UploadStorageEngineCucumberTest,Phase1UploadStorageEngineAwsCliCucumberTest -Dsurefire.failIfNoSpecifiedTests=false` ran twice (before and after the tag edit) with identical results (BUILD SUCCESS, 28 discovered / 0 failures / 0 errors / 19 skipped per class); REQ-UPLOAD-004 executed and passed independently in both `@webclient` and `@awscli` modes, Podman `magrathea-build:fedora42` (JDK 21, Maven 3.9.9, AWS CLI 2.34.29). The java-tester agent refreshed the feature-file tag `@implemented-not-e2e-validated` → `@implemented-and-validated` and removed the feature-level `@partial` tag since no scenario in `phase-1-upload-storage-engine.feature` remains partial/placeholder/config-only/not-implemented; the Gherkin appendix was regenerated and re-verified fresh (`--check` OK, 66 scenarios from 6 feature files).
- [x] Implement REQ-UPLOAD-006 integrity/corruption detection: `StorageEngineIntegrityDetectionTest` (bootstrap) plus previous `Phase1UploadStorageEngineCucumberTest` REQ-UPLOAD-006 WebTestClient evidence prove chunk corruption is detected on read.
- [x] Add `@awscli` runner coverage for REQ-UPLOAD-006: previous `Phase1UploadStorageEngineAwsCliCucumberTest` evidence covers 3 @awscli examples.
...
- [x] Ensure Docker-driven documentation generation includes the report without committing built static web assets. **Done 2026-07-02:** the `Dockerfile` builder stage now copies `scripts/generate-gherkin-requirements-appendix.py` and deterministically regenerates `docs/arc42/generated/gherkin-requirements.adoc` before the documentation JSON/frontend asset conversion, so the built docs always include a freshly regenerated appendix; no built static web assets are committed. Full `docker build` execution is pending an environment with registry network access.
- [x] Add a freshness check command: `python3 scripts/generate-gherkin-requirements-appendix.py --check`.
- [x] Wire the freshness check into CI or Docker quality gates. **Done 2026-07-02:** the `Dockerfile` builder stage runs `python3 scripts/generate-gherkin-requirements-appendix.py --check` before regeneration, failing the image build when the committed appendix is stale. Local gate evidence: `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS (64 scenarios from 6 feature files). Full `docker build` execution is pending an environment with registry network access.
...
- [ ] Plan/implement a richer selected active backend/status contract beyond `/admin/health` when runtime backend evidence is available. **Planned 2026-07-02:** the contract shape (`GET /admin/backend-status`: selected backend, selecting profile/property, catalog summary, storage roots, recovery-scanner summary) is documented in `docs/admin-ui-plan.md`; implementation and validation remain open.
- [x] Plan Vue screens: policy list, policy detail, policy validation/report; device list, device detail; disk-set/topology overview/detail; backend/status dashboard. **Done 2026-07-02:** see `docs/admin-ui-plan.md` (planning only; no `magrathea-ui` code changed).
...
- [x] ADR 0016 is accepted and all docs align with it. Verified 2026-07-02: `docs/adr/0016-*.md` Status is `Accepted`; README documents JaCoCo as the coverage baseline and acknowledges the admin API alongside the S3 object API.
- [x] JaCoCo is documented as the current coverage baseline; Clover is optional/legacy only.
- [x] Admin API presence is documented and no longer contradicted by S3-only wording. Verified 2026-07-02: README and PLAN describe the S3 REST API as the public object API and `/admin/**` as a separate internal/administrative API; no remaining "only public HTTP API is S3"/"S3-only" contradictions were found in README, ARC42, or the test report.
- [x] `StoragePolicy` owns dedup/chunk configuration through `DedupConfig`. Verified 2026-07-02: `DedupConfig` (storage-engine-domain) owns `chunkSize` (validated 4 KiB..1 GiB) and `ChunkAlignment`; `FixedWindowDedupStep` takes its chunk size from the policy spec (commit `9a50ae6`); no independent chunking pipeline step exists.
...
- [x] `MINIO_STANDARD` is explicitly defined, loaded from YAML, and testable. Verified 2026-07-02: `storage-engine-reactive-infrastructure/src/main/resources/storage-policies/minio-standard.yaml` exists with `storageClassId: STANDARD`; Phase 5 evidence (`PersistencePlannerMinioStandardTest`, `MinioStandardIntegrationTest`, commits `b0a5f74`/`0ec84cf`) covers YAML loading, device selection, and deterministic planning.
...
- [x] Admin UI plan covers policy/device/disk-set/backend-status screens and awaits frontend workflow ownership. **Done 2026-07-02:** `docs/admin-ui-plan.md` plans policy list/detail/validation, device list/detail, disk-set/topology overview, and a backend/status dashboard with a planned `GET /admin/backend-status` contract; implementation remains blocked until a frontend workflow/agent owns `magrathea-ui/**`.
