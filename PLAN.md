...
Done: all four acceptance gates met — `YamlStoragePolicyCatalog`/`YamlStorageDeviceCatalog`/`YamlDiskSetCatalog` with robust validation (36 catalog tests); `MINIO_STANDARD` classpath YAML with deterministic plans (`MinioStandardIntegrationTest`); fail-fast on missing external config; context tests for mutually exclusive backends; Admin API integrated via the same catalog beans (`AdminRouterTest`, 9 tests). Local physical EC shards and output-only reconstruction are now separately validated by `REQ-PIPELINE-015/017`. Remaining: no integration test exercises the Admin API through a real YAML-backed Spring Boot context; distributed EC shard placement/transfer and multi-node topology are not runtime-validated; policy schema versioning is documented but not enforced.
...
| Status | `@partial` — fixed A/B/C baseline, bounded current-generation repair, and bounded periodic current-reference anti-entropy are validated. `REQ-CLUSTER-001..005`, `008..014`, and `019..027` are implemented-and-validated, with `014` limited to its internal architecture-only mode and `027` limited to its two fixed A/B/C discovery scenarios; broad cluster `017` remains partial; `006/007/015/016/018` remain not implemented. Local `REQ-PIPELINE-017` reconstruction is implemented-and-validated but is not distributed EC, repair, or EP-10 completion evidence. EP-10 remains the active implementation phase. |
...
|---|---|---|
| **1** | Distributed EC shard placement and direct transfer (`REQ-CLUSTER-015`) | Next requirement-first slice; commit authoritative schema-3 shard locations across the arranged topology and transfer shards over the internal bounded data path. `REQ-CLUSTER-015` remains `@not-implemented`. |
| **2** | Early bounded ADR 0030 kernel and committed-shard fault subset | Planned test-tooling slice after distributed placement/transfer: no-op-default deterministic plan/evidence kernel plus exact committed-shard unavailable/corruption actions, dual non-production activation, finite blast radius, quarantine/restoration, and redacted evidence. No implementation or fault evidence is claimed. |
...
| **9 — active** | **EP-10 S3 cluster** | **High** | **`@partial`; prioritize distributed EC placement/transfer (`REQ-CLUSTER-015`), then the bounded ADR 0030 committed-shard test subset, EC self-healing, shard rebalance, and fenced cleanup; generalized chaos remains final** |
