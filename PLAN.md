...
- [ ] EP-5: CI pipeline green on full gate + appendix check + docker build; offline backup/restore, SIGTERM shutdown validation covers committed state plus single PutObject, multipart UploadPart, CompleteMultipartUpload, cancelled-and-aborted UploadPart cleanup, abort-wins overlapping multipart completion, and two-request concurrent PutObject streams, first single-node DR RTO/RPO rehearsal, object-manifest schema-versioning, and first SLO/alerting rule bundle exist; online/multi-node DR, live alert delivery, broader metadata schema contracts, larger mixed read/write draining, load-level validation, and repeated completion-wins multipart race evidence still pending.
