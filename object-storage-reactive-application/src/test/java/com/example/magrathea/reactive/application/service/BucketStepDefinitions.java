...
public class BucketStepDefinitions {
            ...
            .expectNextMatches(cr -> {
                if (cr instanceof CommandResult<?> cmdResult) {
                    return switch (cmdResult) {
                        case CommandResult.Created<Bucket> created ->
        ...
        StepVerifier.create(result)
            .expectNextMatches(cr -> {
                if (cr instanceof CommandResult<?> cmdResult) {
                    ...
                    return events.stream().anyMatch(e -> e instanceof ObjectStorageEvent.BucketCreated);
                }
                return false;
            })
            .verifyComplete();
    ...
    public void resultIsDeletedBucketWithVersion(long version) {
        ...
        StepVerifier.create(result)
            .expectNextMatches(cr -> {
                if (cr instanceof CommandResult<?> cmdResult) {
                    return switch (cmdResult) {
                        case CommandResult.Deleted<Bucket> deleted ->
    ...
    public void eventBucketDeletedRecorded() {
        ...
        StepVerifier.create(result)
            .expectNextMatches(cr -> {
                if (cr instanceof CommandResult<?> cmdResult) {
                    List<ObjectStorageEvent> events = cmdResult.events();
                    return events.stream().anyMatch(e -> e instanceof ObjectStorageEvent.BucketDeleted);
                }
                return false;
            })
            .verifyComplete();
    ...
    public void setCorsConfiguration(String bucketName) {
        Bucket found = repository.findByName(bucketName).block();
        assertNotNull(found);
        Bucket.Configuration corsConfig = new Bucket.Configuration(
            List.of(new Bucket.Configuration.CorsRule(
                List.of("*"), List.of("GET"), List.of("*"), 3600, List.of(), "cors1"
            ))
        );
        Bucket updated = found.withConfiguration(corsConfig);
    ...
    public void resultIsUpdatedBucketWithVersion(long version) {
        ...
        StepVerifier.create(result)
            .expectNextMatches(cr -> {
                if (cr instanceof CommandResult<?> cmdResult) {
                    return switch (cmdResult) {
                        case CommandResult.Updated<Bucket> updated ->
    ...
    public void eventBucketConfigurationChangedRecorded() {
        ...
        StepVerifier.create(result)
            .expectNextMatches(cr -> {
                if (cr instanceof CommandResult<?> cmdResult) {
                    List<ObjectStorageEvent> events = cmdResult.events();
                    return events.stream().anyMatch(e -> e instanceof ObjectStorageEvent.BucketConfigurationChanged);
                }
                return false;
            })
            .verifyComplete();
            ...
            .expectNextMatches(cr -> {
                if (cr instanceof CommandResult<?> cmdResult) {
                    ...
                    return b.configuration() != null &&
                           b.configuration().corsRules() != null &&
                           b.configuration().corsRules().stream()
