            ...
            .expectNextMatches(cr -> {
                if (cr instanceof CommandResult<?> cmdResult) {
                    return switch (cmdResult) {
                        case CommandResult.Created<S3Object> created ->
        ...
        StepVerifier.create(result)
            .expectNextMatches(cr -> {
                if (cr instanceof CommandResult<?> cmdResult) {
                    ...
                    return events.stream().anyMatch(e -> e instanceof ObjectStorageEvent.ContentDescriptorCreated);
                }
                return false;
            })
            .verifyComplete();
    ...
    public void resultIsDeletedS3ObjectWithVersion(long version) {
        ...
        StepVerifier.create(result)
            .expectNextMatches(cr -> {
                if (cr instanceof CommandResult<?> cmdResult) {
                    return switch (cmdResult) {
                        case CommandResult.Deleted<S3Object> deleted ->
    ...
    public void eventObjectDeletedRecorded() {
        ...
        StepVerifier.create(result)
            .expectNextMatches(cr -> {
                if (cr instanceof CommandResult<?> cmdResult) {
                    List<ObjectStorageEvent> events = cmdResult.events();
                    return events.stream().anyMatch(e -> e instanceof ObjectStorageEvent.ObjectDeleted);
                }
                return false;
            })
            .verifyComplete();
