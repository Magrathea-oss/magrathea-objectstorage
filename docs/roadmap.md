...
| **EP-10** S3 Cluster (Multi-Node) | High | `@partial`; fixed baseline + bounded repair/discovery + architecture contract validated | Fixed A/B/C implements one voter+replica server per JVM, stable UUID roots, mTLS, `N=3/W=2` whole-object publication/failover/restart, consensus-owned current-generation repair, and bounded periodic current-reference anti-entropy (`REQ-CLUSTER-001..005`, `008..013`, `019..027`). Focused `027` passes exactly **2 scenarios / 36 steps**; the bounded seven-point `024` interruption scope remains validated. `REQ-CLUSTER-014` is separately implemented and validated only for its internal repository-rooted source/build architecture mode; it is not S3 or runtime-side-effect evidence. Broad `017` remains partial because rebalance, automated orphan cleanup, and wider healing/topology coverage remain absent. Multipart/conditional/versioned/chunked writes, EC, dynamic membership, and broader partitions also remain absent or unvalidated. |
...
- [ ] EP-10: Overall cluster capability complete — the Java 21 shared real-process gate passes `REQ-CLUSTER-001..005/019/020` (14 scenarios / 188 steps; repair-only `019/020`: 4 / 80), focused `008..013` passes, and the historical 2026-07-13 repair-control run remains 22 scenarios / 210 steps. On 2026-07-14, focused `ReqCluster024CucumberTest` passed 7 / 168, focused `ReqCluster027CucumberTest` passed **2 / 36**, the expanded repair-control suite passed 22 / 294, data-plane passed 4 / 40, and control/TLS regressions passed. `ReqCluster014ArchitectureContractCucumberTest` passed 1 / 17 in its sole internal source/build mode; CI wiring has not yet run. Exact current status: `008..014` and `019..027`, including `024` and `027`, are implemented-and-validated, with `014` architecture-only and `027` bounded to fixed A/B/C current-reference discovery; broad `017` remains partial; `006/007/015/016/018` remain not implemented. The wider cluster scope remains open.
...
With bounded `REQ-CLUSTER-024` repair interruption evidence and bounded `REQ-CLUSTER-027` periodic current-reference discovery complete, the active sequence continues without allowing a cross-cutting roadmap to supersede EP-10:

| Order | EP-10 continuation | Current status / next gate |
|---|---|---|
| **1** | Split `REQ-CLUSTER-017`: rebalance | Next requirement-first slice; define placement-change authority, fencing, and validation independently from repair/discovery. No implementation claim is made; broad `017` remains partial. |
| **2** | Split `REQ-CLUSTER-017`: fenced orphan cleanup | Following requirement-first slice; define prepared-artifact reachability, retention, and deletion fences independently. No implementation claim is made; broad `017` remains partial. |
| **3** | Implement later cluster semantics by explicit slice | `006/007/015/016/018` remain not implemented: multipart/broader writes, EC transfer, dynamic lifecycle, and wider partitions require their own ordered requirements and evidence. |
| **Final, after normal EP work** | ADR 0030 deterministic chaos | Chaos engineering remains accepted as final and deferred until normal Storage Engine and EP-10 paths are complete and stable; `027` is not ADR 0030 execution. |
