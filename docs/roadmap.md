...
| **EP-10** S3 Cluster (Multi-Node) | High | `@partial`; fixed baseline + bounded repair + architecture contract validated | Fixed A/B/C implements one voter+replica server per JVM, stable UUID roots, mTLS, `N=3/W=2` whole-object publication/failover/restart, and consensus-owned current-generation repair (`REQ-CLUSTER-001..005`, `008..013`, `019..026`). The bounded seven-point `024` interruption scope is implemented and validated. `REQ-CLUSTER-014` is separately implemented and validated only for its internal repository-rooted source/build architecture mode; it is not S3 or runtime-side-effect evidence. Broad healing `017` remains partial. Broad periodic anti-entropy, rebalance, orphan cleanup, multipart/conditional/versioned/chunked writes, EC, dynamic membership, and broader partitions remain absent or unvalidated. |
...
- [ ] EP-10: Overall cluster capability complete — the Java 21 shared real-process gate passes `REQ-CLUSTER-001..005/019/020` (14 scenarios / 188 steps; repair-only `019/020`: 4 / 80), focused `008..013` passes, and the historical 2026-07-13 repair-control run remains 22 scenarios / 210 steps. On 2026-07-14 focused `ReqCluster024CucumberTest` passed 7 / 168; the expanded repair-control suite passed 22 / 294, data-plane passed 4 / 40, and control/TLS regressions passed. Also on 2026-07-14, `ReqCluster014ArchitectureContractCucumberTest` passed 1 scenario / 17 steps in its sole internal source/build validation mode; CI is wired but has not run. Exact current status: `008..014` and `019..026`, including `024`, are implemented-and-validated, with `014` limited to architecture evidence; broad `017` remains partial; `006/007/015/016/018` remain not implemented. The wider cluster scope remains open.
...
|---|---|---|
| **1** | Split `REQ-CLUSTER-017`: broad/periodic discovery and anti-entropy | Planned requirement-first slice only; define bounded discovery, scheduling, and observable outcomes before implementation. No implementation claim is made; broad `017` remains partial. |
| **2** | Split `REQ-CLUSTER-017`: rebalance | Planned requirement-first slice only; define placement-change authority, fencing, and validation independently from repair. No implementation claim is made; broad `017` remains partial. |
| **3** | Split `REQ-CLUSTER-017`: fenced orphan cleanup | Planned requirement-first slice only; define prepared-artifact reachability, retention, and deletion fences independently. No implementation claim is made; broad `017` remains partial. |
| **4** | Implement later cluster semantics by explicit slice | `006/007/015/016/018` remain not implemented: multipart/broader writes, EC transfer, dynamic lifecycle, and wider partitions require their own ordered requirements and evidence. |
| **Final, after normal EP work** | ADR 0030 deterministic chaos | Chaos engineering remains final and deferred until the normal Storage Engine and EP-10 paths are complete and stable. |
