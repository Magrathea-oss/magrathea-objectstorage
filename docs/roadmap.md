...
| **EP-10** S3 Cluster (Multi-Node) | High | `@partial`; fixed baseline + bounded repair/discovery + fixed EC transfer validated | Fixed A/B/C implements whole-object `N=3/W=2` publication/failover/restart, consensus-owned current-generation repair, bounded current-reference discovery, and fixed EC 4+2 placement/transfer (`001..005`, `008..015`, `019..027`, with `014` architecture-only and `015` internal-mechanism-only). `REQ-CLUSTER-015` passes **5 scenarios / 40 steps** with A=`0,3`, B=`1,4`, C=`2,5`, real grpc-java/mTLS transfers, six-ack Ratis publication, and restart recovery. Broad cluster `017` remains partial; clustered S3 EC reads, EC self-healing, parameterized EC, rebalance, cleanup, `006/007/016/018`, and production readiness remain absent. Next: bounded committed-shard fault tooling, EC self-healing/read integration, shard rebalance, then fenced cleanup; generalized chaos remains final. |
...
- [ ] EP-10: Overall cluster capability complete — the Java 21 shared real-process gate passes `REQ-CLUSTER-001..005/019/020` (14 scenarios / 188 steps; repair-only `019/020`: 4 / 80), focused `008..013` passes, focused `024` passes 7 / 168, focused `027` passes 2 / 36, and `014` passes 1 / 17 only in its internal source/build mode. Local prerequisite `REQ-PIPELINE-017` separately passes **5 / 46** with all **15 four-of-six** combinations. Fixed internal distributed EC 4+2 `REQ-CLUSTER-015` passes **5 / 40** with deterministic A/B/C placement, real grpc-java/mTLS transfer, six-ack Ratis publication, restart recovery, and fail-closed invalid evidence. Exact current status: `008..015` and `019..027` are implemented-and-validated within their bounded modes; broad cluster `017` remains partial; `006/007/016/018` remain not implemented. Clustered S3 EC reads, self-healing, rebalance, cleanup, and parameterized EC remain open. CI wiring for focused evidence has not yet run.
...
|---|---|---|
| **1 — complete** | Fixed one-stripe EC 4+2 shard placement and direct transfer (`REQ-CLUSTER-015`) | `@implemented-and-validated` internally at 5 scenarios / 40 steps; no clustered S3 EC read or healing claim. |
| **2 — next** | Early bounded ADR 0030 kernel + committed-shard fault subset | Planned no-op-default deterministic test tooling with dual non-production activation, exact committed targets, finite blast radius, quarantine/restoration, and redacted evidence; no implementation claim. |
