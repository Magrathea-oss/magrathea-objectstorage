...
| `S3BucketOperationsHandler` | Bucket lifecycle: create, delete, head, list, location, versioning, directory-bucket listing |
| `S3BucketMetadataHandler` | Bucket metadata: ACL, tagging |
| `S3BucketConfigHandler` | Bucket configuration: CORS, lifecycle, policy, encryption, logging, website, notification, replication, request payment, ownership controls, public access block, accelerate, analytics, inventory, metrics, intelligent-tiering, ABAC, object lock, bucket metadata/table configuration |
| `S3ObjectOperationsHandler` | Object CRUD, copy, multi-delete, rename, torrent, restore, select, Object Lambda response |
| `S3ObjectMetadataHandler` | Object metadata: ACL, tagging, attributes, legal hold, retention, encryption metadata |
| `S3MultipartHandler` | Multipart upload lifecycle: initiate, upload, complete, abort, list |
| `S3SessionHandler` | CreateSession support for Phase F session management |
