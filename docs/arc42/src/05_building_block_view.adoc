...
| `Bucket` | Domain aggregate root: bucket identity, name, region, storage class, versioning, encryption, and dedicated configuration fields per config type (CORS, Lifecycle, Encryption, Logging, Website, Notification, Replication, Policy, PublicAccessBlock, OwnershipControls, RequestPayment, Accelerate, plus multi-instance maps for Analytics, Inventory, Metrics, Intelligent-Tiering). Emits specific ObjectStorageEvent subtypes per config change (e.g., `BucketLifecycleConfigurationChanged`, `BucketNotificationConfigurationChanged`). Each config feature has a dedicated `with*()` / `without*()` method — no generic `withConfiguration()` / flat `Configuration` record. |
