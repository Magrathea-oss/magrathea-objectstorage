...
**Description:** The staged static asset handoff packages the generated admin UI and documentation artifacts into `bootstrap-application` classpath resources.

**Flow:**
1. Browser/Admin UI → bootstrap-application: request `/`, `/assets/*`, or `/docs/*`.
2. bootstrap-application → classpath `static/**`: serves generated Vue assets, ARC42 JSON, ADR JSON, JaCoCo JSON exports, C4 PNG images, and ARC42 image resources.
3. Browser/Admin UI renders documentation and links to `/admin/**` JSON endpoints for operational data.

**Verification boundary:** The static resource handoff verifies files are present and JSON is parseable. It does not change S3 API semantics or storage-engine runtime behavior.
