...
The shared Java 21 real-process gate now passes `REQ-CLUSTER-001..005`, `019`, and `020` in both WebTestClient and AWS CLI modes (14 scenarios, 188 steps); its repair-only `019/020` run passes 4 scenarios / 80 steps. Focused Ratis/gRPC/cross-module scenarios pass for `REQ-CLUSTER-008..013`, and repair-control `021..026` passes 22 scenarios / 210 steps. Root Maven success is only supporting integration evidence. These results do not cover clustered multipart, conditional/versioned writes, erasure coding, dynamic membership, broad periodic anti-entropy, rebalance, orphan cleanup, rolling upgrades, or the broader partition suite.
...
[NOTE]
====
Status: *implementation-informed*. Control persistence, fencing, snapshot migration, lifecycle, and some scheduler reconciliation are validated by `REQ-CLUSTER-021/022/023/025/026`. `REQ-CLUSTER-024` remains partial because the full seven-point real-filesystem/gRPC crash matrix was not executed. No general partition or rolling-upgrade behavior is implied.
====
...
Prepared-artifact intents, orphan cleanup, rebalance, broad periodic anti-entropy, dynamic membership, erasure coding, multipart, and broader partition handling are outside all three implemented bounded flows. Broad `REQ-CLUSTER-017` therefore remains partial.
