...
* EP-5 (Operability & Delivery): CI pipeline, backup/restore, DR, probes, graceful shutdown. First SIGTERM validation now proves a storage-engine process exits without forced termination and preserves a committed S3 object across restart; broader in-flight draining remains open.
