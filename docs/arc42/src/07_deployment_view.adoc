...
.Fixed three-node runtime baseline
image::../../c4/images/FixedThreeNodeRuntimeDeployment.png[Fixed Three-Node Runtime Deployment, width=600]

The cluster profile starts fixed nodes A, B, and C as separate JVMs. Each JVM co-locates one Ratis voter and one whole-object replica server and owns distinct persisted identity, Ratis log/snapshot, object, temporary, and runtime roots. Control and data services have separate ports and lifecycle ownership. The implemented version baseline is Ratis 3.2.2/`ratis-grpc`, grpc-java 1.82.2/`grpc-netty-shaded`, and protobuf 3.25.8.
...
The runtime intentionally covers only unconditional `CreateBucket`, whole-object `PUT`, and whole-object `GET`. Clustered multipart, conditional/versioned or chunked writes, erasure coding, dynamic membership, certificate lifecycle, healing/rebalance/orphan cleanup, rolling upgrade, online multi-node backup/DR, and broader partition behavior remain not implemented or unvalidated. No production distributed-cluster claim is made.
...
* EP-10 (S3 Cluster, multi-node): `@partial`. The fixed A/B/C first slice is implementation-informed and validated for `REQ-CLUSTER-001..005` and `008..013`; `006/007` and `015..018` are not implemented, and `014` remains partial. Broader transfer semantics, dynamic membership, healing/rebalance, and partition behavior remain open.
