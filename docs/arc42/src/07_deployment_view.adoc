...
=== Validated EP-8 Hardened Single-node Evidence Deployment

[NOTE]
====
Status: *implementation-informed / bounded*. This validates evidence wiring and a hardened single-node container only. It is not a cluster deployment or publication claim.
====

The clean evidence run is bound to revision `209b3170b64d3311ba9b773fdb7bd5581519e682` and exact local image ID `sha256:e5b5862948853a613f8b92adc8135fa81b84bdd9f36c9ae8d69b9a6bd6dd7966`; the manifest records `published=false`. Runtime validation passed with UID/GID 10001, read-only root filesystem, explicit writable data and temporary mounts, `no-new-privileges`, capability drop `ALL`, no host PID/IPC/network/UTS namespaces, and no container-engine socket. User-namespace remapping was unavailable in the local engine and is recorded as unavailable rather than claimed.
...
=== Planned Initial Cluster Deployment (ADR 0027)

[NOTE]
====
Status: *planned / absent*. No multi-node deployment or Ratis validation exists.
====

The initial topology has three storage-node deployments. Each co-locates one logical Cluster Control Plane voter and one Cluster Data Plane role while keeping control and data responsibilities distinct. Voters should span the best available independent `zone → rack → host` failure domains; a three-voter group tolerates one unavailable voter, not two. Each node persists its UUID and cluster state separately from replaceable process/container identity.

Only internal mTLS-authenticated protobuf gRPC/HTTP2 is permitted between nodes. Static seed addresses provide bootstrap contact and do not define membership. Writable mounts must be explicit for storage data, Ratis log/snapshots, and runtime state; planned hardening validation requires non-root execution, dropped unnecessary capabilities, no privilege escalation, and a read-only root filesystem outside those mounts.

The embedded Apache Ratis deployment remains conditional on a spike covering persistence/snapshots, membership change, leader transfer/restart, mTLS integration, upgrade compatibility, bounded resources, Reactor bridging, and partition/fault behavior. Dedicated control-plane nodes or an external consensus service remain future reconsideration points if co-location or Ratis validation is unacceptable.
...
* EP-8 (Cluster Architecture ADR & Supply Chain): `@implemented-and-validated` only for accepted architecture/evidence wiring. Clean-revision SBOM/license/image-hardening evidence and CI retention are validated without publication; current OWASP vulnerability status is unknown/error.
* EP-10 (S3 Cluster, multi-node): `@absent` after the accepted architecture-only ADR — membership, consensus, quorum transfer, failover, healing, rebalance, and partition behavior still require multi-node semantic and fault-injection scenarios
