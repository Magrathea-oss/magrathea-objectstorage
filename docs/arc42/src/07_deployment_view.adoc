...
* EP-5 (Operability & Delivery): CI pipeline, backup/restore, DR, probes, graceful shutdown. First SIGTERM validation now proves a storage-engine process exits without forced termination and preserves a committed S3 object across restart. First offline backup/restore validation copies the storage root, simulates primary data loss, restores, restarts, and reads the object back. First single-node DR rehearsal validates RTO 30 seconds and RPO as the last completed offline backup. First manifest schema-versioning validation writes version `1`, accepts legacy omitted schema as compatibility version `0`, and rejects unsupported future versions. First SLO/alerting bundle validation ships Prometheus and Loki starter rules plus an alert response runbook; live Alertmanager/Loki delivery, broader in-flight draining, and online/multi-node DR remain open.
