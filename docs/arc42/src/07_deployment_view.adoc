...
* EP-5 (Operability & Delivery): CI pipeline, backup/restore, DR, probes, graceful shutdown. SIGTERM validation now proves a storage-engine process exits without forced termination, preserves a committed S3 object across restart, and drains active PutObject and multipart UploadPart streams with HTTP 200, including two concurrent PutObjects; restart validation verifies the single and concurrent object checksums and completes the multipart upload from the drained part ETag before final checksum verification. First offline backup/restore validation copies the storage root, simulates primary data loss, restores, restarts, and reads the object back. First single-node DR rehearsal validates RTO 30 seconds and RPO as the last completed offline backup. First manifest schema-versioning validation writes version `1`, accepts legacy omitted schema as compatibility version `0`, and rejects unsupported future versions. First SLO/alerting bundle validation ships Prometheus and Loki starter rules plus an alert response runbook; live Alertmanager/Loki delivery, larger mixed read/write concurrent draining, in-flight multipart completion/cancellation races, load-level shutdown validation, and online/multi-node DR remain open.
