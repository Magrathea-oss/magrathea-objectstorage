...
The implemented cluster runtime covers unconditional `CreateBucket`, whole-object `PUT`/`GET`, bounded repair of a missing or corrupt whole-object target, bounded periodic inspection of fixed A/B/C whole-object obligations, and fixed internal EC 4+2 placement/transfer/publication. `ClusterNodeRuntime` composes `ClusterEcWriteCoordinator` over the existing per-peer grpc-java/mTLS clients and Ratis control port. Clustered S3 EC reads, EC self-healing, parameterized EC, multipart, conditional/versioned or chunked cluster writes, dynamic membership, certificate lifecycle, wider healing topologies, rebalance, orphan cleanup, rolling upgrade, online multi-node backup/DR, and broader partitions remain unimplemented or unvalidated. No production distributed-cluster claim is made.
...
* EP-10 (S3 Cluster, multi-node): `@partial`. Fixed A/B/C is implementation-informed and validated for `REQ-CLUSTER-001..005`, `008..015`, and `019..027`, with `014` architecture-only and `015` internal fixed EC placement/transfer only. Clustered S3 EC reads and broad cluster `017` remain incomplete. The sequence is bounded ADR 0030 committed-shard test tooling, EC self-healing/read integration, shard rebalance, and cleanup; parameterized EC remains later and generalized chaos final. Whole-object replication remains valid below the arranged EC minimum, and inter-cluster/NAS-backed replication remains later and NAS-dependent.
