...
The implemented runtime covers unconditional `CreateBucket`, whole-object `PUT`/`GET`, bounded repair of a missing or corrupt target already named by the current reference, and bounded periodic inspection of fixed A/B/C current named obligations. Clustered multipart, conditional/versioned or chunked writes, erasure coding, dynamic membership, certificate lifecycle, wider anti-entropy/healing topologies, rebalance, orphan cleanup, rolling upgrade, online multi-node backup/DR, and broader partition behavior remain not implemented or unvalidated. No production distributed-cluster claim is made.
...
Prepared-artifact intents and orphan cleanup, rebalance, wider anti-entropy/healing topologies, dynamic membership, erasure coding, multipart, ADR 0030 general chaos, and broader partition handling remain outside the deployment change.
