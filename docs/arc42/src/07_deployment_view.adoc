...
* Exposes ports 8080 and 8081 and defines an Admin API healthcheck against `/admin/health`
* Validated first slice: image build, Admin API health smoke, S3 ListBuckets XML and bucket/object PUT/GET smoke, and no Spring Boot generated-password banner in build/runtime logs
...
* Runtime stage: `alpine`, non-root user, no JRE/JDK installed, ports 8080 and 8081 exposed
* Validated first slice: image build, Admin API health smoke, S3 ListBuckets XML/JSON and bucket/object PUT/GET smoke, no Spring Boot generated-password banner, no native reflection/shared-arena runtime errors, and no `java`/`javac` in the runtime image
