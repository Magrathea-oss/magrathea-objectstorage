...
> - **Latest full Maven gate (2026-07-12, local sandbox — Java 25.0.3):** `mvn -B --no-transfer-progress test` at the repository root passed with BUILD SUCCESS across all 13 reactor modules. The full-gate reports contain **113 reports, 1,403 discovered report entries, 1,089 executed tests/examples, 314 skipped or tag-filtered entries, 0 failures, and 0 errors**. Docker-required REQ-OPS-023/025 evidence is reported separately because those opt-in runners are excluded from ordinary Maven. Execution counts are distinct from the generated Gherkin inventory and shared features may be rediscovered by multiple filtered runners.
...
| Gherkin requirements/specs appendix generator check | n/a | 0 | n/a | 445 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 445 scenarios from 28 feature files grouped by ADR 0020 stakeholder audience. |
