...
> - **Latest full Maven gate (2026-07-11, local sandbox — Java 25.0.3):** `mvn -B --no-transfer-progress test` at the repository root passed with BUILD SUCCESS across all 13 reactor modules. Fresh Surefire XML contains **104 reports, 1,325 discovered report entries, 1,051 executed tests/examples, 274 skipped or tag-filtered entries, 0 failures, and 0 errors**. Execution counts are distinct from the generated Gherkin inventory and shared features may be rediscovered by multiple filtered runners.
...
> - Phase 3 now has pipeline-unit and WebTestClient Cucumber evidence for selected staged-pipeline requirements: REQ-PIPELINE-001 executes in both modes; REQ-PIPELINE-002 executes its deterministic 256 MiB pipeline-unit example but remains `@partial` because WebTestClient is `@not-implemented`; REQ-PIPELINE-003 and REQ-PIPELINE-004 remain `@partial`; REQ-PIPELINE-005 executes in pipeline-unit mode while WebTestClient cancellation remains `@not-implemented`; REQ-PIPELINE-006 is implemented-and-validated for its declared pipeline-unit mode. Streaming requirements REQ-PIPELINE-007..012 are implemented-and-validated by Cucumber static-architecture specs; REQ-PIPELINE-013 is implemented-and-validated only for the finite-demand adapter boundary covered by its runtime runner. This does not close the staged-pipeline scope.
...
| Phase 3 WebTestClient Cucumber specs | 3 | 0 | 18 | 21 | `Phase3ReactivePipelineCucumberTest` PASS: executes REQ-PIPELINE-001, REQ-PIPELINE-003, and the supported REQ-PIPELINE-004 chunk-fault example through the S3 adapter. |
...
| REQ-PIPELINE-002 | Bounded demand/backpressure/no whole-object aggregation | Pipeline-unit Cucumber; WebTestClient pending | ⚠️ Partial | A deterministic 256 MiB demand-controlled body passes through the production staged pipeline and real filesystem with measured live source-buffer retention and request signals bounded by four, ordered manifest/length validation, exact streamed SHA-256 readback, and anti-aggregation source guards. The WebTestClient example remains `@not-implemented`; no client-driven HTTP memory claim is made. |
| REQ-PIPELINE-003 | Read pipeline manifest chunk order | Pipeline-unit + WebTestClient Cucumber | ⚠️ Partial | Pipeline-unit proves first-chunk streaming under controlled demand. WebTestClient preserves deterministic REQ-FS-003/004 XML failures through a bounded, non-retaining integrity preflight before opening a separate response stream; it does not claim first-byte latency, single-pass filesystem I/O, or active per-chunk demand control. |
...
| REQ-PIPELINE-005 | Cancellation event and cleanup | Pipeline-unit Cucumber | ⚠️ Partial | Pipeline-unit proves cancellation and cleanup; the WebTestClient example remains explicitly `@not-implemented`. |
...
| Phase 3 mode summary | Reactive pipeline requirements | Pipeline-unit/WebTestClient selections (REQ-PIPELINE-001..006); static architecture Cucumber (REQ-PIPELINE-007..012); runtime boundary Cucumber (REQ-PIPELINE-013) | Mixed status; EP-3 remains open | REQ-PIPELINE-002 WebTestClient evidence and WebTestClient cancellation remain open; REQ-PIPELINE-002/003/004/005 remain partial. No AWS CLI runner validates the staged internal pipeline contract. |
