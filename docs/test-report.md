...
> - **Latest full Maven gate (2026-07-10, local sandbox — JDK 21.0.11, Maven 3.9.9, AWS CLI 2.34.32):** `mvn test` at the repo root passed with BUILD SUCCESS across all 13 reactor modules: Surefire XML reports record **1138 discovered tests/examples, 990 passed/executed, 0 failures, 0 errors, 148 skipped/excluded**. The log was checked for Spring Boot's generated-password banner and none was found.
...
> - Phase 3 reactive pipeline remains **implemented-not-e2e-validated** for REQ-PIPELINE-001 through REQ-PIPELINE-006 (unit/application validation only; no Phase 3 WebTestClient, AWS CLI, or end-to-end Cucumber validation has been executed yet). Streaming requirements REQ-PIPELINE-007..012 are implemented-and-validated by Cucumber static-architecture specs; REQ-PIPELINE-007/008 are also guarded by `ReactiveUploadStreamingArchitectureTest`.
...
> - **EP-5 operability probe, shutdown, backup/restore, DR, and alerting slice (2026-07-10):** `PhaseEp5OperabilityRequirementsCucumberTest` passed with 11 Business Need scenarios, 0 failures/errors/skips, validating Admin API `/admin/live` liveness, `/admin/ready` storage-catalog readiness, fail-closed `503 not-ready` behavior when required catalogs are unavailable (`REQ-OPS-001..003`), `REQ-OPS-004` SIGTERM shutdown/recovery evidence, `REQ-OPS-009` SIGTERM draining of an active 524,288-byte streaming PutObject with restart byte-count/checksum verification, `REQ-OPS-010` draining of an active 524,288-byte multipart UploadPart followed by restart completion and final checksum verification, `REQ-OPS-011` draining of two concurrent 262,144-byte PutObjects with restart checksum verification for both, `REQ-OPS-012` draining of an active CompleteMultipartUpload with restart checksum verification of the assembled object, `REQ-OPS-005` offline backup/restore evidence, `REQ-OPS-006` single-node DR objectives: RTO 30 seconds and RPO last completed offline backup, and `REQ-OPS-008` shipped SLO/alerting bundle coverage for Admin probes, S3 smoke, storage capacity, backup age, generated-password regressions, and manifest-schema regressions. `docs/runbooks/graceful-shutdown.md`, `docs/runbooks/backup-restore.md`, `docs/runbooks/disaster-recovery.md`, and `docs/runbooks/slo-alerts.md` record the validated procedures and their scope. The EP-5 storage migration spec (`REQ-OPS-007`) also passed, proving current object manifests declare schema version `1`, legacy manifests without the key remain readable as compatibility version `0`, and unsupported future schema version `999` is rejected. The focused CI gate including EP-1, EP-2, EP-3, EP-5, KA-5, and Phase 5 runners passed with 196 tests/examples, 0 failures/errors, and 76 skipped. Generated-password banner count: 0.
...
| Phase 3 static architecture Cucumber specs | 6 | 0 | 3 | 9 | `Phase3ReactivePipelineStaticArchitectureSpecsCucumberTest` PASS: enforces REQ-PIPELINE-007..012 for PutObject, fixed-window dedup, non-range/range GetObject, UploadPart/UploadPartCopy, and multipart part-store streaming constraints. |
...
| EP-5 Admin API operability, graceful shutdown, backup/restore, DR, and SLO alerting | 11 | 0 | 0 | 11 | `PhaseEp5OperabilityRequirementsCucumberTest` PASS: Admin API `/admin/live` returns liveness status and link to readiness; `/admin/ready` returns readiness status with storage-policy, storage-device, and disk-set catalog components ready; missing catalogs return `503 not-ready`; SIGTERM shutdown exits without forced termination and preserves a committed S3 object across storage-engine process restart; single PutObject, multipart UploadPart, CompleteMultipartUpload, and two concurrent PutObject requests are drained with HTTP 200 before exit; the PutObject survives restart directly, while the drained part ETag completes the multipart upload after restart and the final byte count/checksum matches; offline backup/restore copies the storage root, simulates primary data loss, restores, and reads the committed S3 object back; single-node DR rehearsal validates RTO 30 seconds and RPO last completed offline backup; shipped Prometheus/Loki alert rules and `docs/runbooks/slo-alerts.md` cover SLO objectives, first-response links, generated-password regression detection, and manifest-schema regression detection. Generated-password count: 0. |
| EP-5 storage-engine manifest schema migration specs | 1 | 0 | 0 | 1 | `PhaseEp5StorageMigrationSpecsCucumberTest` PASS: current manifest files declare `manifest.schemaVersion=1`, legacy manifests without the key remain readable as compatibility version `0`, and unsupported future schema version `999` is rejected. |
...
| Gherkin requirements/specs appendix generator check | n/a | 0 | n/a | 395 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 395 scenarios from 22 feature files (13 `Business Need`, 9 `Ability`) grouped by ADR 0020 stakeholder audience. |
...
| Gherkin requirements/specs ARC42 appendix (`docs/arc42/generated/gherkin-requirements.adoc`) | `python3 scripts/generate-gherkin-requirements-appendix.py --check` | PASS: generated appendix is fresh; 395 scenarios from 22 feature files | Documentation-generation evidence only. The report summarizes explicit scenario tags and does not invent implementation or validation results. |
...
| Phase 3 mode summary | Reactive pipeline requirements | Unit/application (REQ-PIPELINE-001..006); static architecture Cucumber (REQ-PIPELINE-007..012) | ⚠️ Implemented-not-e2e-validated for 001–006; ✅ 007..012 validated via static architecture specs | No Phase 3 WebTestClient, AWS CLI, or end-to-end Cucumber runner executes the staged pipeline scenarios yet. |
