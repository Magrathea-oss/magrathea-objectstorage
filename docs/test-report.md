...
> - **Latest full Maven gate (2026-07-11, local sandbox — Java 25.0.3):** `mvn -B --no-transfer-progress test` at the repository root passed with BUILD SUCCESS across all 13 reactor modules. Fresh Surefire XML contains **104 reports, 1,349 discovered report entries, 1,059 executed tests/examples, 290 skipped or tag-filtered entries, 0 failures, and 0 errors**. Execution counts are distinct from the generated Gherkin inventory and shared features may be rediscovered by multiple filtered runners.
...
| Phase 3 pipeline-unit Cucumber specs | 13 | 0 | 14 | 27 | `Phase3PipelineUnitSpecsCucumberTest` PASS: executes selected REQ-PIPELINE-001..006 and REQ-PIPELINE-014/015 examples through the production staged pipeline and real filesystem adapters; includes deterministic 256 MiB bounded-demand evidence, cleanup, instrumentation, whole-object namespace validation, and physical EC data/parity shards. |
| Phase 3 WebTestClient Cucumber specs | 7 | 0 | 20 | 27 | `Phase3ReactivePipelineCucumberTest` PASS: executes REQ-PIPELINE-001..005 selected examples plus REQ-PIPELINE-014 whole-object namespace and REQ-PIPELINE-015 physical EC shard validation through the S3 adapter. |
