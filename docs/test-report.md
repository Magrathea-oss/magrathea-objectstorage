...
> - **EP-5 operability probe, shutdown, and backup/restore slice (2026-07-10):** `PhaseEp5OperabilityRequirementsCucumberTest` passed with 5 Business Need scenarios, 0 failures/errors/skips, validating Admin API `/admin/live` liveness, `/admin/ready` storage-catalog readiness, fail-closed `503 not-ready` behavior when required catalogs are unavailable (`REQ-OPS-001..003`), `REQ-OPS-004` SIGTERM shutdown/recovery evidence, and `REQ-OPS-005` offline backup/restore evidence: a storage-engine S3 process writes a committed object, stops for a backup window, copies the storage root to a backup location, loses the primary root, restores from backup, restarts, and reads the object back. `docs/runbooks/backup-restore.md` records the validated offline procedure and its scope. Generated-password banner count: 0.
...
| EP-5 Admin API operability, graceful shutdown, and backup/restore | 5 | 0 | 0 | 5 | `PhaseEp5OperabilityRequirementsCucumberTest` PASS: Admin API `/admin/live` returns liveness status and link to readiness; `/admin/ready` returns readiness status with storage-policy, storage-device, and disk-set catalog components ready; missing catalogs return `503 not-ready` with component statuses `not-configured`; SIGTERM shutdown exits without forced termination and preserves a committed S3 object across storage-engine process restart; offline backup/restore copies the storage root, simulates primary data loss, restores, and reads the committed S3 object back. Generated-password count: 0. |
...
| Gherkin requirements/specs appendix generator check | n/a | 0 | n/a | 388 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 388 scenarios from 21 feature files (13 `Business Need`, 8 `Ability`) grouped by ADR 0020 stakeholder audience. |
...
| Gherkin requirements/specs ARC42 appendix (`docs/arc42/generated/gherkin-requirements.adoc`) | `python3 scripts/generate-gherkin-requirements-appendix.py --check` | PASS: generated appendix is fresh; 388 scenarios from 21 feature files | Documentation-generation evidence only. The report summarizes explicit scenario tags and does not invent implementation or validation results. |
