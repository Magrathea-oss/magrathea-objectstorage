...
> - **Root JVM Docker image gate (2026-07-10):** `Dockerfile` now uses public ECR mirrored Maven/Temurin base images, copies the full `scripts/` gate set, runs the Gherkin appendix freshness gate before packaging, regenerates docs/UI assets, and packages without Maven fail-never mode. `docker build --network=host -f Dockerfile -t magrathea-objectstorage:jvm .` passed. Container smoke validation with host networking passed for `/admin/health`, S3 ListBuckets XML, bucket `PUT`, object `PUT`, and object `GET`; build/runtime logs contained 0 Spring Boot generated-password banners.
...
> - **EP-5 operability probe slice (2026-07-10):** `PhaseEp5OperabilityRequirementsCucumberTest` passed with 2 Business Need scenarios, 0 failures/errors/skips, validating Admin API `/admin/live` liveness and `/admin/ready` storage-catalog readiness (`REQ-OPS-001/002`). `mvn -pl admin-api-adapter -am test` also passed after adding the routes. Generated-password banner count: 0.
...
| Gherkin requirements/specs appendix generator check | n/a | 0 | n/a | 385 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 385 scenarios from 21 feature files (13 `Business Need`, 8 `Ability`) grouped by ADR 0020 stakeholder audience. |
