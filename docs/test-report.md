...
> - **EP-5 operability probe and shutdown slice (2026-07-10):** `PhaseEp5OperabilityRequirementsCucumberTest` passed with 4 Business Need scenarios, 0 failures/errors/skips, validating Admin API `/admin/live` liveness, `/admin/ready` storage-catalog readiness, fail-closed `503 not-ready` behavior when required catalogs are unavailable (`REQ-OPS-001..003`), and `REQ-OPS-004` SIGTERM shutdown/recovery evidence: a storage-engine S3 process writes a committed object, exits within 10 seconds without forced termination, emits no Spring generated-password banner, restarts with the same filesystem root, and reads the object back. `mvn -pl admin-api-adapter -am test` also passed after adding the routes. Generated-password banner count: 0.
...
| EP-5 Admin API operability and graceful shutdown | 4 | 0 | 0 | 4 | `PhaseEp5OperabilityRequirementsCucumberTest` PASS: Admin API `/admin/live` returns liveness status and link to readiness; `/admin/ready` returns readiness status with storage-policy, storage-device, and disk-set catalog components ready; missing catalogs return `503 not-ready` with component statuses `not-configured`; SIGTERM shutdown exits without forced termination and preserves a committed S3 object across storage-engine process restart. `mvn -pl admin-api-adapter -am test` PASS after adding routes. Generated-password count: 0. |
...
| Gherkin requirements/specs appendix generator check | n/a | 0 | n/a | 387 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 387 scenarios from 21 feature files (13 `Business Need`, 8 `Ability`) grouped by ADR 0020 stakeholder audience. |
...
| Gherkin requirements/specs ARC42 appendix (`docs/arc42/generated/gherkin-requirements.adoc`) | `python3 scripts/generate-gherkin-requirements-appendix.py --check` | PASS: generated appendix is fresh; 387 scenarios from 21 feature files | Documentation-generation evidence only. The report summarizes explicit scenario tags and does not invent implementation or validation results. |
