...
> - Phase 6 distributed-readiness policy remains **implemented-not-e2e-validated** for its broad modeled domain scope. EP-10 now executes only the fixed `N=3/W=2` first-slice placement/publication path with real replication and multi-node S3 evidence; dynamic membership, EC transfer, healing/rebalance job runners, broader topology behavior, and general distributed readiness remain absent. Distributed production readiness is not claimed.
...
| Gherkin requirements/specs appendix generator check | n/a | 0 | n/a | 518 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 518 scenarios from 34 feature files grouped by ADR 0020 stakeholder audience. |
...
| EP-10 opt-in shared real-process S3 gate (`REQ-CLUSTER-001..005`, `-Pep10-cluster-tests`) | 10 | 0 | 0 | 10 scenarios / 108 steps | This dedicated profile gate, not ordinary `mvn test`, executes the five shared requirement scenarios once through WebTestClient and once through AWS CLI against real A/B/C child JVMs. Covers consensus bucket visibility, fixed `N=3/W=2` whole-object publication, exact-byte failover, both no-publication failures, and complete restart. |
| EP-10 focused mechanism gates (`REQ-CLUSTER-008..013`) | PASS | 0 | tag-filtered scenarios excluded per runner | Focused reports | Real Ratis control/TLS, grpc-java streaming/TLS/cancellation/deadline, and cross-module publication runners pass. `REQ-CLUSTER-014` remains partial; these reports do not validate later EP-10 capabilities. |
...
| Phase 6 distributed readiness (`storage-engine-domain`) | Spec source: `s3-reactive-api-adapter/src/test/features/specs/phase-6-distributed-readiness.feature`; domain implementation: `storage-engine-domain/src/main/java/com/example/magrathea/storageengine/domain/distributed/`; tests: `DistributedPlacementPlannerTest`, `QuorumPolicyTest`, `AntiEntropyPlannerTest`, `RebalancePlannerTest`, `DistributedReadinessReporterTest`; gates: `mvn -B -pl storage-engine-domain test --no-transfer-progress`, `mvn -B test --no-transfer-progress` | Storage-engine domain gate PASS: 164 tests, 0 failures, 0 errors, 0 skipped. Full Maven gate PASS / BUILD SUCCESS: 883 test cases observed, 0 failures, 0 errors, 11 skipped. | `@implemented-not-e2e-validated` for the broad modeled domain/unit scope. Deterministic placement, health-aware membership decisions, quorum decisions, anti-entropy/healing plans, rebalance plans, and readiness classification are modeled and unit validated. EP-10 separately validates only fixed A/B/C `N=3/W=2` whole-object replication and publication; real dynamic membership, EC transfer, healing/rebalance job runners, wider topology behavior, and general multi-node readiness remain absent. No public storage-engine object API endpoints were added; S3 object behavior remains exposed through S3-compatible APIs only. |
...
| Gherkin requirements/specs ARC42 appendix (`docs/arc42/generated/gherkin-requirements.adoc`) | `python3 scripts/generate-gherkin-requirements-appendix.py --check` | PASS: generated appendix is fresh; 518 scenarios from 34 feature files; all 18 `REQ-CLUSTER-*` statuses are present | Documentation-generation evidence only. The report summarizes explicit scenario tags and does not invent observed test results. |
...
| Phase 6 mode summary | Broad distributed-readiness modeled domain scope | Domain/unit, with a separate bounded EP-10 runtime slice | ⚠️ Broad scope remains modeled/unit validated only | EP-10 separately validates fixed A/B/C `N=3/W=2` whole-object replication and consensus reference publication through WebTestClient/AWS CLI. Dynamic membership, EC, healing/rebalance execution, wider topology, and general distributed readiness remain absent. |
