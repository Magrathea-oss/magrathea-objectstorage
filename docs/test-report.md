...
> - **Latest full Maven gate (2026-07-13, local sandbox — Java 25.0.3):** `mvn -B --no-transfer-progress test -Dstyle.color=never` at the repository root passed with BUILD SUCCESS across all 14 reactor projects in 12:38. The full-gate reports contain **115 Surefire reports, 1,448 discovered report entries, 1,109 executed tests/examples, 339 skipped or tag-filtered entries, 0 failures, and 0 errors**. Docker-required REQ-OPS-023/025 evidence is reported separately because those opt-in runners are excluded from ordinary Maven. Execution counts are distinct from the generated Gherkin inventory and shared features may be rediscovered by multiple filtered runners.
...
> - **Gherkin requirements/specs ARC42 appendix:** regenerated after the EP-7 status update; `python3 scripts/generate-gherkin-requirements-appendix.py --check` passes and reports **486 scenarios from 31 feature files**. `REQ-ADMIN-032..037` now show `@implemented-and-validated`, not stale `@partial`, while the appendix remains grouped by ADR 0020 stakeholder audience (`Business Need` vs `Ability`).
...
| Maven full test gate (all modules) | 1109 | 0 | 339 | 1448 | Latest full Maven gate (2026-07-13): `mvn -B --no-transfer-progress test -Dstyle.color=never` at repository root PASS / BUILD SUCCESS in 12:38; all 14 reactor projects; 115 full-gate Surefire XML reports aggregate 1,448 discovered tests/examples, 1,109 passed/executed, 0 failures, 0 errors, and 339 skipped/tag-filtered. Docker-required REQ-OPS-023/025 results are recorded separately. |
...
| Gherkin requirements/specs ARC42 appendix (`docs/arc42/generated/gherkin-requirements.adoc`) | `python3 scripts/generate-gherkin-requirements-appendix.py --check` | PASS: generated appendix is fresh; 486 scenarios from 31 feature files; REQ-ADMIN-032..037 show `@implemented-and-validated` | Documentation-generation evidence only. The report summarizes explicit scenario tags and does not invent implementation or validation results. |
