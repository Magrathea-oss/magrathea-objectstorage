...
> - **Latest full Maven gate (2026-07-12, local sandbox — Java 25.0.3):** `mvn -B --no-transfer-progress test` at the repository root passed with BUILD SUCCESS across all 13 reactor modules. The full-gate reports contain **115 Surefire reports, 1,440 discovered report entries, 1,109 executed tests/examples, 331 skipped or tag-filtered entries, 0 failures, and 0 errors**. Docker-required REQ-OPS-023/025 evidence is reported separately because those opt-in runners are excluded from ordinary Maven. Execution counts are distinct from the generated Gherkin inventory and shared features may be rediscovered by multiple filtered runners.
...
> - **Quality/security evidence slice (2026-07-12, evidence only):** `mvn -B -Pcoverage clean verify --no-transfer-progress` produced the reactor-wide JaCoCo baseline at revision `c3fe8bb1` (48,191/60,631 instructions, 1,957/3,484 branches, 8,868/11,367 lines, 2,671/3,316 methods, and 570/635 classes covered). No coverage threshold is enforced. OWASP Dependency-Check 12.2.2 failed closed while updating NVD data: after downloading 40,000 of 364,882 records, NVD returned HTTP 429. The analysis records `scanStatus=error`, `clean=null`, and 8 scan errors. The JSON/HTML vulnerability reports were not generated, so the zero finding counters are **not findings evidence** and neither a zero-vulnerability nor clean-scan claim is permitted. `REQ-QUAL-001..004` remain `@partial` pending executable semantic validation and a complete Dependency-Check scan.
...
| Maven full test gate (all modules) | 1109 | 0 | 331 | 1440 | Latest full Maven gate (2026-07-12): `mvn -B --no-transfer-progress test` at repository root PASS / BUILD SUCCESS; all 13 reactor modules; 115 full-gate Surefire XML reports aggregate 1,440 discovered tests/examples, 1,109 passed/executed, 0 failures, 0 errors, and 331 skipped/tag-filtered. Docker-required REQ-OPS-023/025 results are recorded separately. |
...
| Gherkin requirements/specs appendix generator check | n/a | 0 | n/a | 486 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 486 scenarios from 31 feature files grouped by ADR 0020 stakeholder audience, including the four `@partial` quality/security evidence scenarios. |
...
| EP-7 frontend Vitest | 72 | 0 | 0 | 72 | 14 files PASS: shell contracts/static boundaries, extension composition, application/browser adapters, themes, template, and accessibility components. |
| EP-7 Playwright/axe | 39 | 0 | 0 | 39 | 13 real-browser scenarios at each Chromium viewport width 360/768/1440 PASS. |
| EP-7 reproducible packaging | PASS | 0 | n/a | n/a | Deterministic extension removal, isolated dual-product builds from one shell, and canonical Docker `frontend-packaging-validation` build-twice SHA-256 comparison passed. |
...
| OWASP Dependency-Check 12.2.2 | n/a | Scan error | n/a | Incomplete | NVD HTTP 429 after 40,000/364,882 records; `scanStatus=error`, `clean=null`, 8 errors. No JSON/HTML vulnerability report, findings result, or clean-scan claim. |
...
| Gherkin requirements/specs ARC42 appendix (`docs/arc42/generated/gherkin-requirements.adoc`) | `python3 scripts/generate-gherkin-requirements-appendix.py --check` | PASS: generated appendix is fresh; 486 scenarios from 31 feature files | Documentation-generation evidence only. The report summarizes explicit scenario tags and does not invent implementation or validation results. |
...
> The detailed module-level table below is retained as a previous Phase 10 breakdown. The current aggregate evidence is the 2026-07-12 full Maven gate summarized above: `mvn -B --no-transfer-progress test` passed with BUILD SUCCESS across all 13 reactor modules, and **115 Surefire XML reports record 1,440 discovered tests/examples, 1,109 passed/executed, 0 failures, 0 errors, and 331 skipped/tag-filtered**. Some rows below may remain historical snapshots; use the Summary and Current Verified Results sections above for current validation evidence.
...
The canonical gate passed all 14 reactor projects. Surefire XML contained 1,448 tests/scenarios, 0 failures, 0 errors, and 339 skipped. JaCoCo analyzed all 12 production modules; cross-module execution data from the S3/Cucumber test module was included in the aggregate.
...
### Dependency vulnerability evidence (2026-07-12)
...
The observed unauthenticated run downloaded 40,000 of 364,882 NVD records (11%) and then received HTTP 429. Maven exited 1. The analyzer therefore recorded `scanStatus=error`, `clean=null`, and 8 scan errors. Dependency-Check generated neither canonical vulnerability report. The analysis's zero unsuppressed/suppressed counters result from the absent report and must **not** be interpreted as vulnerability findings, zero vulnerabilities, or a clean scan.

`REQ-QUAL-001..004` remain `@partial`: the artifacts provide implementation-informed evidence, but the scenarios do not yet have executable semantic validation and the OWASP scan is incomplete.
