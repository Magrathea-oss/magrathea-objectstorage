...
> - **Latest full Maven gate (2026-07-11, local sandbox — Java 25.0.3):** `mvn -B --no-transfer-progress test` at the repository root passed with BUILD SUCCESS across all 13 reactor modules. Fresh Surefire XML contains **104 reports, 1,345 discovered report entries, 1,054 executed tests/examples, 291 skipped or tag-filtered entries, 0 failures, and 0 errors**. Execution counts are distinct from the generated Gherkin inventory and shared features may be rediscovered by multiple filtered runners.
...
| Phase 3 pipeline-unit Cucumber specs | 10 | 0 | 16 | 26 | `Phase3PipelineUnitSpecsCucumberTest` PASS: executes selected REQ-PIPELINE-001..006 and REQ-PIPELINE-014 examples through the production staged pipeline and real filesystem adapters; includes deterministic 256 MiB bounded-demand/live-buffer evidence, canonical UUID/sidecar interoperability, cleanup behavior, instrumentation, and the 8 MiB no-forced-splitting regression. |
...
| Gherkin requirements/specs appendix generator check | n/a | 0 | n/a | 420 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 420 scenarios from 25 feature files (14 `Business Need`, 11 `Ability`) grouped by ADR 0020 stakeholder audience. |
...
| Gherkin requirements/specs ARC42 appendix (`docs/arc42/generated/gherkin-requirements.adoc`) | `python3 scripts/generate-gherkin-requirements-appendix.py --check` | PASS: generated appendix is fresh; 420 scenarios from 25 feature files | Documentation-generation evidence only. The report summarizes explicit scenario tags and does not invent implementation or validation results. |
