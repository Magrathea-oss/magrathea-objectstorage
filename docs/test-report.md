...
> - **Current ordinary root Maven gate after EP-10 composition and the EP-8 feature correction (2026-07-13, dirty local working tree):** `mvn -B --no-transfer-progress test -Dstyle.color=never` at the repository root passed with BUILD SUCCESS across **18/18 reactor projects**. Its **124 Surefire XML reports contain 1,589 discovered tests/examples, 1,148 executed, 441 skipped or tag-filtered, 0 failures, and 0 errors**. The opt-in EP-10 14-scenario / 188-step shared gate, 4-scenario / 80-step repair-only run, and focused mechanism/repair-control gates remain separate and are not folded into these totals; focused EP-8 supply-chain and Docker-required REQ-OPS-023/025 evidence are also separate. This dirty-working-tree run is supporting integration evidence only: it is neither acceptance-eligible supply-chain evidence nor a refresh of the historical clean-revision EP-8 SBOM, license, image, hardening, or OWASP packet.
...
> - Phase 3 now has pipeline-unit and WebTestClient Cucumber evidence for selected staged-pipeline requirements: REQ-PIPELINE-001, deterministic 256 MiB REQ-PIPELINE-002, and client-cancellation REQ-PIPELINE-005 execute in both modes and are `@implemented-and-validated`; REQ-PIPELINE-003 and REQ-PIPELINE-004 are `@implemented-and-validated` for their declared modes; REQ-PIPELINE-006 is implemented-and-validated for its declared pipeline-unit mode. Streaming requirements REQ-PIPELINE-007..012 are implemented-and-validated by Cucumber static-architecture specs; REQ-PIPELINE-013 is implemented-and-validated only for the finite-demand adapter boundary covered by its runtime runner. REQ-PIPELINE-014 and REQ-PIPELINE-015 now validate the whole-object namespace and physical EC shards in pipeline-unit and WebTestClient modes. REQ-PIPELINE-016 validates pre-commit temporary-file verification, and all declared EP-3 scenarios are implemented-and-validated.
...
| Phase 3 pipeline-unit Cucumber specs | 14 | 0 | 16 | 30 | `Phase3PipelineUnitSpecsCucumberTest` PASS: executes selected REQ-PIPELINE-001..006 and REQ-PIPELINE-014..016 examples; includes 256 MiB bounded single-pass reads, cleanup, instrumentation, typed namespaces, physical EC shards, and fault-injected verified-write rejection. |
...
| Current ordinary root Maven gate | 1148 | 0 | 441 | 1589 | 2026-07-13 root `mvn test` PASS across 18/18 reactor projects; 124 Surefire XML reports. The dirty-working-tree result excludes separately reported focused/opt-in EP-8 and EP-10 evidence and is supporting integration evidence only, not EP-10 semantic or acceptance supply-chain evidence. |
...
| Gherkin requirements/specs appendix generator check | n/a | 0 | n/a | 526 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 526 scenarios from 34 feature files grouped by ADR 0020 stakeholder audience. |
| EP-8 architecture contract | 13 | 0 | 0 | 13 expanded scenarios | `PhaseEp8ArchitectureContractCucumberTest` PASS with 114 steps. ADR 0027 is accepted architecture-only; contract validation does not implement EP-10 behavior. |
...
| EP-10 focused mechanism gates (`REQ-CLUSTER-008..013`) | PASS | 0 | tag-filtered scenarios excluded per runner | Focused reports | Real Ratis control/TLS, grpc-java streaming/TLS/cancellation/deadline, cross-module publication, and post-repair data-plane regression pass. `REQ-CLUSTER-014` remains partial. |
| EP-10 repair-control gate (`REQ-CLUSTER-021..026`) | 22 | 0 | 0 | 22 scenarios / 210 steps | Deduplication, consensus lifecycle, claim/session fencing, snapshot migration, current-generation obsolescence, and bounded scheduler reconciliation pass. `021/022/023/025/026` are implemented-and-validated; `024` remains partial because all seven crash points were not executed with full real filesystem and gRPC side effects. |
...
| Phase 6 distributed readiness (`storage-engine-domain`) | Spec source: `s3-reactive-api-adapter/src/test/features/specs/phase-6-distributed-readiness.feature`; domain implementation: `storage-engine-domain/src/main/java/com/example/magrathea/storageengine/domain/distributed/`; tests: `DistributedPlacementPlannerTest`, `QuorumPolicyTest`, `AntiEntropyPlannerTest`, `RebalancePlannerTest`, `DistributedReadinessReporterTest`; gates: `mvn -B -pl storage-engine-domain test --no-transfer-progress`, `mvn -B test --no-transfer-progress` | Storage-engine domain gate PASS: 164 tests, 0 failures, 0 errors, 0 skipped. Full Maven gate PASS / BUILD SUCCESS: 883 test cases observed, 0 failures, 0 errors, 11 skipped. | `@implemented-not-e2e-validated` for the broad modeled domain/unit scope. Deterministic placement, health-aware membership decisions, quorum decisions, anti-entropy/healing plans, rebalance plans, and readiness classification are modeled and unit validated. EP-10 separately validates fixed A/B/C `N=3/W=2` whole-object replication/publication and bounded current-generation repair; real dynamic membership, EC transfer, broad periodic anti-entropy, rebalance, orphan cleanup, wider topology behavior, and general multi-node readiness remain absent. No public storage-engine object API endpoints were added; S3 object behavior remains exposed through S3-compatible APIs only. |
...
| Gherkin requirements/specs ARC42 appendix (`docs/arc42/generated/gherkin-requirements.adoc`) | `python3 scripts/generate-gherkin-requirements-appendix.py --check` | PASS: generated appendix is fresh; 526 scenarios from 34 feature files; all `REQ-CLUSTER-001..026` statuses are present | Documentation-generation evidence only. The report summarizes explicit scenario tags and does not invent observed test results. |
...
| Phase 3 mode summary | Reactive pipeline requirements | Pipeline-unit/WebTestClient selections (REQ-PIPELINE-001..006, 014..016); static architecture Cucumber (REQ-PIPELINE-007..012, 016); runtime boundary Cucumber (REQ-PIPELINE-013) | ✅ EP-3 implemented and validated | All declared EP-3 scenarios pass their required modes. The internal staged-pipeline contract does not require an AWS CLI runner; externally observable S3 upload/read integrity remains shared with the Phase 1 AWS CLI scenarios. |
...
| Phase 6 mode summary | Broad distributed-readiness modeled domain scope | Domain/unit, with a separate bounded EP-10 runtime slice | ⚠️ Broad scope remains modeled/unit validated only | EP-10 separately validates fixed A/B/C `N=3/W=2` whole-object publication and bounded current-generation repair through WebTestClient/AWS CLI. Dynamic membership, EC, broad anti-entropy/rebalance/cleanup, wider topology, and general distributed readiness remain absent. |
...
> The detailed module-level table below is retained as a previous Phase 10 breakdown. The current aggregate evidence is the 2026-07-13 ordinary root gate summarized above: `mvn -B --no-transfer-progress test -Dstyle.color=never` passed with BUILD SUCCESS across **18/18 reactor projects**, and **124 Surefire XML reports record 1,589 discovered tests/examples, 1,148 passed/executed, 0 failures, 0 errors, and 441 skipped/tag-filtered**. The opt-in EP-10 14-scenario / 188-step shared result, 4-scenario / 80-step repair-only result, focused EP-10 mechanisms/repair control, and historical focused EP-8 supply-chain runner are not folded into those ordinary Maven totals. The dirty-working-tree root pass is not acceptance supply-chain evidence. Some rows below remain historical snapshots; use the Summary and Current Verified Results sections above for current validation evidence.
...
The latest ordinary full-root gate passed 18/18 reactor projects. Its 124 Surefire XML reports contained 1,589 discovered tests/scenarios, 1,148 executed, 0 failures, 0 errors, and 441 skipped/tag-filtered. That dirty-working-tree `mvn test` run is supporting integration evidence only; it did not run the opt-in EP-10 acceptance profile, refresh supply-chain artifacts, or refresh the JaCoCo baseline below. The coverage figures remain the historical 2026-07-12 `c3fe8bb1` baseline, where JaCoCo analyzed all 12 then-current production modules and included cross-module execution data from the S3/Cucumber test module.
