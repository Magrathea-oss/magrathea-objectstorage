...
> - **Latest full Maven gate (2026-07-13, local sandbox — Java 25.0.3):** `mvn -B --no-transfer-progress test -Dstyle.color=never` at the repository root passed with BUILD SUCCESS across all 14 reactor projects in 12:40. Excluding the stale opt-in `PhaseEp8SupplyChainEvidenceCucumberIT.xml` left by the earlier targeted failure and not discovered by ordinary Surefire, the full-gate report set contains **116 Surefire reports, 1,470 discovered report entries, 1,122 executed tests/examples, 348 skipped or tag-filtered entries, 0 failures, and 0 errors**. The focused EP-8 supply-chain runner separately passed **9/9 after repair** and is not folded into these ordinary Maven totals. Docker-required REQ-OPS-023/025 evidence is also reported separately because those opt-in runners are excluded from ordinary Maven. Execution counts are distinct from the generated Gherkin inventory and shared features may be rediscovered by multiple filtered runners.
...
| Maven full test gate (all modules) | 1122 | 0 | 348 | 1470 | Latest full Maven gate (2026-07-13): `mvn -B --no-transfer-progress test -Dstyle.color=never` at repository root PASS / BUILD SUCCESS in 12:40; all 14 reactor projects; excluding the stale opt-in `PhaseEp8SupplyChainEvidenceCucumberIT.xml` from the earlier targeted failure, 116 full-gate Surefire XML reports aggregate 1,470 discovered tests/examples, 1,122 passed/executed, 0 failures, 0 errors, and 348 skipped/tag-filtered. The separately repaired EP-8 supply-chain runner passed 9/9 and is not included in these ordinary Maven totals; Docker-required REQ-OPS-023/025 results are also recorded separately. |
...
| Gherkin requirements/specs appendix generator check | n/a | 0 | n/a | 500 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 500 scenarios from 32 feature files grouped by ADR 0020 stakeholder audience. |
...
| Gherkin requirements/specs ARC42 appendix (`docs/arc42/generated/gherkin-requirements.adoc`) | `python3 scripts/generate-gherkin-requirements-appendix.py --check` | PASS: generated appendix is fresh; 500 scenarios from 32 feature files; EP-8 statuses and explicit EP-10-runtime-absent boundary are current | Documentation-generation evidence only. The report summarizes explicit scenario tags and does not invent implementation or validation results. |
...
> The detailed module-level table below is retained as a previous Phase 10 breakdown. The current aggregate evidence is the 2026-07-13 full Maven gate summarized above: `mvn -B --no-transfer-progress test -Dstyle.color=never` passed with BUILD SUCCESS across all 14 reactor projects in 12:40, and, after excluding the stale opt-in `PhaseEp8SupplyChainEvidenceCucumberIT.xml` from the earlier targeted failure, **116 Surefire XML reports record 1,470 discovered tests/examples, 1,122 passed/executed, 0 failures, 0 errors, and 348 skipped/tag-filtered**. The focused EP-8 supply-chain runner's separate 9/9 pass after repair is not folded into those ordinary Maven totals. Some rows below may remain historical snapshots; use the Summary and Current Verified Results sections above for current validation evidence.
...
The latest ordinary full-root gate passed all 14 reactor projects. After excluding the stale opt-in `PhaseEp8SupplyChainEvidenceCucumberIT.xml` from the earlier targeted failure, its 116 Surefire XML reports contained 1,470 discovered tests/scenarios, 1,122 executed, 0 failures, 0 errors, and 348 skipped/tag-filtered; the focused runner's separate 9/9 pass after repair is not included. JaCoCo analyzed all 12 production modules; cross-module execution data from the S3/Cucumber test module was included in the aggregate.
...
The EP-8 current-revision run for `209b3170b64d3311ba9b773fdb7bd5581519e682` downloaded 190,000 of 364,891 NVD records and then received HTTP 429. Retained acceptance evidence records `scanStatus=error`, `clean=null`, and 8 errors. This result is deliberately monitoring-only `unknown/error`; absent or zero counters must **not** be interpreted as vulnerability findings, zero vulnerabilities, or a clean scan. EP-8 performed no dependency remediation or suppression change.
