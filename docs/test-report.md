...
> - **Latest full Maven gate (2026-07-11, local sandbox — Java 25.0.3):** `mvn -B --no-transfer-progress test` at the repository root passed with BUILD SUCCESS across all 13 reactor modules. Fresh Surefire XML contains **105 reports, 1,370 discovered report entries, 1,067 executed tests/examples, 303 skipped or tag-filtered entries, 0 failures, and 0 errors**. Execution counts are distinct from the generated Gherkin inventory and shared features may be rediscovered by multiple filtered runners.
...
> - **EP-4 REQ-SCRUB-001/002 first integrity-hygiene slice (2026-07-11):** the opt-in periodic scrub job incrementally hashes final persisted artifact representations, including compressed bytes and encrypted ciphertext, correlates findings with typed artifacts, owning manifests, and applied COMPRESS/CRYPT transformations, supports report-only or quarantine policy, leaves healthy bytes unchanged, suppresses duplicate post-quarantine findings, and retains the latest run report. Focused Cucumber: **9 discovered / 5 executed / 4 tag-filtered, 0 failures/errors**. Infrastructure reactor gate: **261 tests, 0 failures/errors**. GC, dedup reference counting, quota/ENOSPC handling, deep decrypt/decompress probes, and repair remain open.
...
| Maven full test gate (all modules) | 1067 | 0 | 303 | 1370 | Latest full Maven gate (2026-07-11): `mvn -B --no-transfer-progress test` at repository root PASS / BUILD SUCCESS; all 13 reactor modules; 105 fresh Surefire XML reports aggregate 1,370 discovered tests/examples, 1,067 passed/executed, 0 failures, 0 errors, and 303 skipped/tag-filtered. |
...
| Gherkin requirements/specs appendix generator check | n/a | 0 | n/a | 423 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 423 scenarios from 25 feature files (14 `Business Need`, 11 `Ability`) grouped by ADR 0020 stakeholder audience. |
...
| REQ-GC-001..004, REQ-QUOTA-001/002, REQ-CAPACITY-001 | Reclamation, dedup reference counting, quotas, and ENOSPC behavior | Pending | ⛔ Not implemented | Remain the next EP-4 slices; no completion is inferred from scrubbing support. |
