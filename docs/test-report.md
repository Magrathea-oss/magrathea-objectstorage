...
> - **Gherkin requirements ARC42 appendix:** `python3 scripts/generate-gherkin-requirements-appendix.py --check` passes and reports **344 scenarios from 15 feature files** after ADR 0021 migrated the legacy `features/awscli/` and `features/object-store/` suites into classified `requirements/` / `specs/` structure.
...
| Gherkin requirements appendix generator check | n/a | 0 | n/a | 344 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 344 scenarios from 15 requirement feature files after ADR 0021 migration. |
