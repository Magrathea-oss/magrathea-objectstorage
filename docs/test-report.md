...
> - **Latest full Maven gate (2026-07-12, local sandbox — Java 25.0.3):** `mvn -B --no-transfer-progress test` at the repository root passed with BUILD SUCCESS across all 13 reactor modules. The full-gate reports contain **110 reports, 1,392 discovered report entries, 1,084 executed tests/examples, 308 skipped or tag-filtered entries, 0 failures, and 0 errors**. Docker-required REQ-OPS-023/025 evidence is reported separately because those opt-in runners are excluded from ordinary Maven. Execution counts are distinct from the generated Gherkin inventory and shared features may be rediscovered by multiple filtered runners.
...
| Maven full test gate (all modules) | 1084 | 0 | 308 | 1392 | Latest full Maven gate (2026-07-12): `mvn -B --no-transfer-progress test` at repository root PASS / BUILD SUCCESS; all 13 reactor modules; 110 full-gate Surefire XML reports aggregate 1,392 discovered tests/examples, 1,084 passed/executed, 0 failures, 0 errors, and 308 skipped/tag-filtered. Docker-required REQ-OPS-023/025 results are recorded separately. |
...
| Gherkin requirements/specs appendix generator check | n/a | 0 | n/a | 429 scenarios | `python3 scripts/generate-gherkin-requirements-appendix.py --check` PASS; generated ARC42 appendix is fresh: 429 scenarios from 26 feature files grouped by ADR 0020 stakeholder audience. |
...
| Gherkin requirements/specs ARC42 appendix (`docs/arc42/generated/gherkin-requirements.adoc`) | `python3 scripts/generate-gherkin-requirements-appendix.py --check` | PASS: generated appendix is fresh; 423 scenarios from 25 feature files | Documentation-generation evidence only. The report summarizes explicit scenario tags and does not invent implementation or validation results. |
