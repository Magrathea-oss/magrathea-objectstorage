...
REQ-PIPELINE-014 and REQ-PIPELINE-015 have pipeline-unit and WebTestClient evidence for the whole-object namespace and physical EC shard persistence. The GetObject decision is single-pass payload streaming after metadata-only validation: upload commit verifies temporary-file bytes against the incoming digest before atomic publication, clients validate later reads against committed checksum/ETag metadata, and periodic at-rest detection/repair belongs to EP-4 scrubbing. EP-4 is implemented and validated for the declared single-node scope. REQ-SCRUB-001/002 covers incremental final-representation checksums, including compressed bytes and encrypted ciphertext. REQ-GC-001..004 covers restart-safe typed reclamation and shared dedup reachability. REQ-QUOTA-001/002 and REQ-CAPACITY-001 cover durable logical-byte accounting, concurrent reservations, Admin reporting, and deterministic/native ENOSPC behavior through WebTestClient and real AWS CLI modes. Distributed quota coordination belongs to EP-10; deep plaintext transform probes and EC reconstruction are outside this decision's declared scope.
